...
public sealed record SupplierReturnShipmentSnapshot(Guid Id, Guid SupplierReturnAuthorizationId, string ReturnNumber, Guid? SourceVendorBillId, string SourceVendorBillNumber, Guid VendorId, string VendorName, string ShipmentNumber, DateOnly ShippedOn, string Status, decimal TotalAmount, decimal VendorCreditAmount, bool CreatesVendorCredit, decimal SourceAppliedAmount, decimal AppliedAmount, decimal RefundedAmount, decimal AvailableAmount, string TransactionCurrency, decimal TransactionAvailableAmount, Guid WarehouseId, Guid BinId, string Location, Guid JournalEntryId, Guid? ReversalJournalEntryId, string ConcurrencyToken, IReadOnlyList<SupplierReturnShipmentLineSnapshot> Lines, IReadOnlyList<SupplierReturnCreditApplicationSnapshot> Applications, IReadOnlyList<SupplierReturnCreditRefundSnapshot> Refunds);
...
public sealed record ProjectsWorkspace(
    ...
    IReadOnlyList<ProjectPhaseSnapshot>? Phases = null,
    IReadOnlyList<ProjectCostCodeSnapshot>? CostCodes = null,
    IReadOnlyList<ProjectBudgetAllocationSnapshot>? BudgetAllocations = null);
...
public sealed record ProjectJobSnapshot(
    ...
    string BillingMethod = "TimeAndMaterials",
...
public sealed record ProjectPhaseSnapshot(Guid Id, Guid ProjectJobId, string ProjectJobNumber, Guid? ParentProjectPhaseId, string Code, string Name, string Kind, string Description, DateOnly? StartsOn, DateOnly? EndsOn, bool IsActive, string ConcurrencyToken);
public sealed record ProjectCostCodeSnapshot(Guid Id, string Code, string Name, string Category, string Description, bool IsActive, string ConcurrencyToken);
public sealed record ProjectBudgetAllocationSnapshot(Guid Id, Guid ProjectJobId, string ProjectJobNumber, Guid? ProjectPhaseId, string ProjectPhaseCode, Guid? ProjectCostCodeId, string ProjectCostCode, Guid? AccountId, string AccountNumber, DateOnly PeriodStart, DateOnly PeriodEnd, decimal BudgetAmount, decimal ForecastAmount, string Notes, string ConcurrencyToken);
