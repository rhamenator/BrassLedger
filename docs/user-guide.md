...
Create and maintain projects from the Projects page. Assign an active project on journal, invoice, bill, quote, sales-order, requisition, purchase-order, timecard, and payroll earning lines. Posted expense and revenue lines drive actual cost, revenue, and margin; unreceived purchase-order value drives commitments. The ledger drill-down shows the 250 most recent tagged lines while portfolio totals continue to use the complete posted ledger. Close a job only after its open orders, requisitions, and timecards are resolved. Closing and reopening require a reason and preserve audit evidence. See [Project and job accounting](project-accounting-guide.md) for accounting behavior, QuickBooks mapping, and current limitations.
