...
| Current evidence | `9e03097` implements native foreign customer-deposit and vendor-advance refunds with retained transaction/carrying/cash/rate/FX/reversal evidence, provider migrations, guided AR/AP selection, partial/final rounding, gain/loss, missing-rate rejection and newest-to-oldest exact reversal. Release build, SQLite/default 198 plus 10 expected skips, PostgreSQL 208/208, API 37/37, components 16/16, provider drift, vulnerability and whitespace gates pass. Chromium is 36/37 because the Ledger baseline predates the already-checkpointed remeasurement panel; explicit foreign-refund browser interaction and that deliberate baseline update remain before sub-slice 1 is `Verified`. |
...
- Browser status: 36/37 passed. The sole failure is the Ledger visual baseline: the actual capture is valid but 504 pixels taller because it contains the previously implemented foreign-currency remeasurement panel absent from the committed baseline. The actual image was preserved at `/home/rich/temp/brassledger-ledger-20260827.actual.png`. Review/update that baseline deliberately, then add explicit browser interaction coverage for foreign customer and vendor refund rate selection before marking refund sub-slice 1 verified.
...
1. **Implemented; browser/concurrency acceptance remains — Foreign unapplied-payment refunds:** `9e03097` extends `SubledgerAdjustment` to retain transaction amount/currency, deposit-or-advance carrying amount, cash base amount, selected direct/inverse settlement-rate identity/factor/effective date/source/reference, realized gain/loss, reversal accounting date, and concurrency evidence. A refund request amount is in the payment currency. Both unapplied balances, refund-date cash translation, proportional/final carrying release, explicit FX posting, base compatibility, exact ordered reversal, provider migrations/backfill/downgrade prohibition, API and guided UI are implemented. Before marking `Verified`, add explicit browser interaction for foreign customer/vendor rate selection and hostile concurrent-refund evidence, cover the remaining negative matrix (wrong pair/date/company, insufficient cash, reconciliation), and deliberately reconcile the stale Ledger visual baseline described in the restart handoff.
...
- `9e03097` — implements native foreign unapplied customer-deposit and vendor-advance refunds with retained rate and carrying evidence, realized gain/loss, exact ordered reversal, provider migrations and guided UI. All non-browser gates pass; it remains an implementation checkpoint rather than a verified slice until the explicit browser/concurrency/negative matrix and stale Ledger baseline are resolved.
