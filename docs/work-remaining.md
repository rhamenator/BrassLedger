...
| Last pushed commit | `76fdd16 fix(reporting): bind ownership events to transitions` |
| In progress | Consolidated acquisition/disposal, ownership-change, and profit-attribution accounting |
| Current evidence | The `5f78f2c` ownership-event checkpoint is gate-complete from one clean out-of-tree Release assembly: build 0 warnings/errors; SQLite/default 194 plus 10 expected skips; PostgreSQL 204/204; API 35/35; components 16/16; isolated Chromium 37/37; both EF models clean; changed-file whitespace clean; all 12 projects free of known vulnerable direct/transitive NuGet packages. The pushed `76fdd16` follow-up additionally ties acquisition, step-acquisition, continuing-control-change and loss-of-control dates/percentages to actual adjacent ownership periods; requires step-acquisition prior-interest fair value; makes intra-period departure/reentry gaps explicit; and has clean focused build, API, SQLite lifecycle, transition posting/statement/CSV, and Chromium acquisition evidence. |

## Immediate queue

1. **In progress — CONSOL-ACQUISITION:** extend the pushed controlled event ledger with schema-v2 line-item purchase-price allocation, consideration/deferred-tax/measurement-period detail, and its guided browser/export evidence.
...
| CONSOL-ACQUISITION | In progress | The typed reviewed event ledger, posting/reversal lifecycle, statement integration and controlled exports are proven by `5f78f2c`; `76fdd16` adds transition-date coupling and posted step/change/loss statement/CSV vectors. Still require line-item identifiable asset/liability, consideration, deferred-tax and measurement-period purchase-price-allocation detail plus guided browser/export evidence before this slice can be `Verified`. |
