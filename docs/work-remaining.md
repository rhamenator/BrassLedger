...
| Current evidence | `9e03097` implements native foreign customer-deposit and vendor-advance refunds with retained transaction/carrying/cash/rate/FX/reversal evidence, provider migrations, guided AR/AP selection, partial/final rounding, gain/loss, missing-rate rejection and newest-to-oldest exact reversal. `413b182` proves the rendered customer/vendor foreign-payment and dated-refund-rate workflows and stabilizes visual defaults across calendar-day rollover. Release build, SQLite/default 198 plus 10 expected skips, PostgreSQL 208/208, API 37/37, components 16/16, Chromium 38/38, provider drift, vulnerability and whitespace gates pass. Hostile concurrent-refund and the remaining focused negative matrix are the only acceptance work before sub-slice 1 can be `Verified`. |
...
1. **Implemented; concurrency/negative acceptance remains — Foreign unapplied-payment refunds:** `9e03097` extends `SubledgerAdjustment` to retain transaction amount/currency, deposit-or-advance carrying amount, cash base amount, selected direct/inverse settlement-rate identity/factor/effective date/source/reference, realized gain/loss, reversal accounting date, and concurrency evidence. A refund request amount is in the payment currency. Both unapplied balances, refund-date cash translation, proportional/final carrying release, explicit FX posting, base compatibility, exact ordered reversal, provider migrations/backfill/downgrade prohibition, API and guided UI are implemented; `413b182` proves customer/vendor rendered workflows and restores a clean 38/38 browser gate. Before marking `Verified`, add hostile concurrent-refund evidence and the explicit refund-path negative matrix listed in the restart handoff.
...
- `9e03097` — implements native foreign unapplied customer-deposit and vendor-advance refunds with retained rate and carrying evidence, realized gain/loss, exact ordered reversal, provider migrations and guided UI. `413b182` supplies the subsequent browser evidence; hostile concurrency and the focused negative matrix still prevent a `Verified` designation.
- `413b182` — adds rendered foreign customer-deposit and vendor-advance refund workflows, confirms payment/refund rate selection and translated results, updates the approved remeasurement-aware Ledger baseline, and makes screenshot defaults deterministic across day rollover; Chromium passes 38/38. Hostile concurrency and the focused negative matrix remain.
