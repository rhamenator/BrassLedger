...
Projects can use as-billed, cost-to-cost, reviewed manual-percentage, or completed-contract recognition. As-billed projects retain invoice-driven revenue; controlled cumulative WIP schedules recognize revenue independently of billing for the other three methods. A retained preview records contract, cost, estimate, completion, earned revenue, gross billing, prior and desired contract positions, the incremental revenue true-up, source fingerprint, and project concurrency token. Preparation, independent approval, separate posting, rejection, correction, cancellation, latest-schedule-only exact reversal, closed-period controls, and contract-asset and contract-liability reconciliation are enforced. The same revenue account must remain in use while unreversed WIP history exists, and completed-contract WIP must be reversed before its project can reopen. SQLite and PostgreSQL have versioned migrations with lost-history adoption and destructive downgrade protection. Multiple performance obligations, variable consideration, expected-loss provisions, phases/cost codes, and full historical project export remain incomplete; see the [project accounting guide](project-accounting-guide.md).

Verification completed on 2026-08-26:

- Release solution build: succeeded with zero warnings and zero errors.
- SQLite/default infrastructure suite: 192 tests passed; nine PostgreSQL-only tests were skipped in that run (201 total).
- Disposable PostgreSQL 16 infrastructure suite: 201 of 201 tests passed with zero skips, including concurrent change-order approval, concurrent same-source project billing, and concurrent WIP preparation reserving their controls exactly once.
- API integration suite: 33 of 33 tests passed, including mismatched routes, antiforgery enforcement, billing preview/save/cancellation, self-approval rejection, independent approval, and revised totals.
- Web component suite: 10 of 10 tests passed, including the retainage aging table and control-account reconciliation warning.
- Chromium end-to-end suite: 31 of 31 tests passed, including distinct least-privilege WIP preparer, approver, and poster users; posting and exact reversal; and visually inspected Overview, Ledger, Projects, and Reporting baselines.
- SQLite and PostgreSQL EF model-drift checks: no pending model changes.
...
| Advanced project accounting | Partially implemented | Controlled change orders, source-derived billing, contract caps, retainage withholding/release, separate retainage receivables/aging/reconciliation, cumulative WIP, independent revenue recognition, exact reversal, control reconciliation, and separation of duties are implemented; multiple performance obligations, variable consideration, expected-loss provisions, phases/cost codes, and full historical export remain open. |
...
- Project dimensioning, lifecycle, derived actuals, purchase commitments, recent-ledger drill-down, controlled change orders, source-derived billing, separate retainage receivables, release, aging, cumulative WIP, independent revenue recognition, exact reversal, and control reconciliation are implemented. Multiple performance obligations, variable consideration, expected-loss provisions, phased budgets/cost codes, and full historical ledger export are not yet complete.
