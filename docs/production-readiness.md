...
- Chromium end-to-end suite: 30 of 30 tests passed, including distinct least-privilege preparer and approver users and the visually inspected Ledger and Projects baselines.
- SQLite and PostgreSQL EF model-drift checks: no pending model changes.
...
- Project dimensioning, lifecycle, derived actuals, purchase commitments, recent-ledger drill-down, controlled change orders, source-derived billing, separate retainage receivables, release, aging, and control reconciliation are implemented. WIP, automated revenue recognition independent of billing, phased budgets, and full historical ledger export are not yet complete.
