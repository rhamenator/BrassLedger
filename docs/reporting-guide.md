...
Consolidated statement packages can be downloaded as CSV, Excel, or PDF from **Reporting** after running either the current-period or comparative package. Excel is the full working-paper format with statement, source-detail, ownership-event, and independently approved framework-disclosure sheets. PDF is the controlled presentation format with repeated headers and wrapped statement, source, ownership measurement/posting, financing-liability, supplier-finance, and narrative evidence. Both formats display the reporting currency, exact dates, reconciliation, warnings, and completion state. Disclosure pages retain framework edition, JSON schema, content SHA-256, approval identity/time, source references, and period-specific content. Posted acquisition, disposal, ownership-change, and attribution entries appear in statement source detail under their event type and retained reference; their versioned measurement schedules, postings, review identities, sources, and SHA-256 identity also travel with current and comparative JSON, CSV, Excel, and PDF packages. Schema-2 acquisition output additionally exposes consideration components, identifiable assets and liabilities, deferred tax, measurement-period dates and signed adjustment history with supporting source references. An **Incomplete** banner means the file is not ready for external use; downloading it does not waive any unresolved accounting warning.
