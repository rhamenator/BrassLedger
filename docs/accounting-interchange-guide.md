...
The chart export uses `Account Name`, `Type`, `Detail Type`, and `Account Number`, matching Intuit's current chart-import fields. The journal export uses `Journal No.`, `Journal Date`, `Account Name`, `Journal/Description`, `Debits`, and `Credits`, with extra reference, line-description, `Project / Job`, `Project Phase`, `Cost Code`, `Department`, and `Class` columns available for mapping. Project aliases are `Project/Job`, `Project Job`, and `Project`; `Class` is deliberately not a project alias. Phase aliases include `Phase`, `Project Task`, `Task`, and `Work Package`. Department aliases are `Location` and `Business Unit`, while class aliases are `Tracking Class` and `QuickBooks Class`.

A project must identify one active same-company BrassLedger project by job number or unique exact name; a phase must belong to that project, and the cost code must be active in the same company. Phase or cost-code values require a project. A department or class must identify one active same-company value by code or unique exact name and be effective on the journal date. Unknown, ambiguous, closed, inactive, expired, future-effective, wrong-type, or foreign-company references reject the batch rather than losing attribution. QuickBooks Online performs an explicit mapping and review step during upload, so review every mapped field in QuickBooks rather than assuming header recognition alone. In particular, confirm whether the QuickBooks location field should map to the controlled BrassLedger Department value for that company.

Invoice CSV requires `Invoice No.`, `Customer`, `Invoice Date`, `Due Date`, and a positive `Item Amount` on every line. `Customer` may be an existing BrassLedger customer number or an unambiguous exact name. Optional `Quantity` and `Rate` must either both be blank or multiply to `Item Amount` to the cent. Optional `Income Account` may be an active revenue-account number or unambiguous exact name; blank values map to the active company's configured **Default revenue** operational account. Optional `Project / Job`, `Project Phase`, `Cost Code`, `Department`, and `Class` use the same validation and aliases as journal imports, are evaluated on the invoice date, and persist on each invoice line. Optional `Tax Amount` must be blank or zero. All lines sharing an invoice number must use the same customer and dates.
...
- products/services, class and location master synchronization, taxable invoices, bills, payments, credit memos, journal entries, and opening balances through the API adapter (controlled Department and Class values are implemented for journal and zero-tax invoice CSV interchange);
