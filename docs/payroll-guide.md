...
The protected employee record separately stores mailing city and state rather than assuming that a residence tax jurisdiction is the employee's postal address. Name components and the complete encrypted mailing address flow into the immutable W-2 baseline and later corrections. The EFW2C encoder implements the SSA's 1,024-byte RCA, RCE, RCW, RCT, and RCF positional records, conditional original/correct money pairs, reconciliation totals, ASCII and field-length validation, and strict specification-tax-year matching against the exact approved publication. Reviewed layouts cover [SSA EFW2C tax year 2025, version 2.3](https://www.ssa.gov/employer/efw/25efw2c.pdf) and the [initial tax-year 2026 publication dated July 10, 2026](https://www.ssa.gov/employer/efw/26efw2c.pdf). The 2026 Employer Signature PIN and Treasury tipped-occupation fields remain blank when unused, as that publication permits; BrassLedger does not silently reuse another year's approval.

Each company maintains its own protected BSO submitter and employer-contact configuration. Activating a specification requires the exact year-specific SSA PDF URL, file checksum, retrieval date, compatible reviewed layout identifier, review notes, and approval. An approved W-2c/W-3c package can produce one immutable encrypted file only when its tax year exactly matches that active specification. Downloads recheck the stored SHA-256. The file begins in `GeneratedForAccuWage`; an authorized reviewer must retain the AccuWage evidence reference and outcome notes, after which the validation result cannot be overwritten. `AccuWageValidated` records successful structural testing, not SSA receipt or acceptance.

These files are filing-ready review data, not transmitted returns or printable official forms. BrassLedger now produces source-locked 941-X and W-2c/W-3c correction packages and immutable SSA EFW2C files for AccuWage review, but does not yet produce IRS Modernized e-File messages, original SSA EFW2 files, red-ink or approved substitute forms, qualified-tip/overtime reporting, every W-2 box/code, or jurisdiction return layouts. The payloads deliberately retain `RequiresProfessionalReview` and transmission-status flags until those workflows, external validation, and independent payroll-tax review are complete.
