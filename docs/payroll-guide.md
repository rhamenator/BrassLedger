...
Posting creates one durable employee payment record for every employee calculation line. Those records must sum exactly to run net pay or posting is rejected. An employee with a complete, enabled direct-deposit election receives a `DirectDeposit` instruction; everyone else receives a `Check` instruction. This is an internal instruction and audit record, not a NACHA file. NACHA generation remains unavailable until the company funding account has separately protected origin-routing, account, company-identification, and effective-entry configuration.
...
The lifecycle, protected fields, auditable timecards, detailed earning entry, multi-location earning allocation, 2026 FIT/FICA engine, reconciled liability/remittance subledger, employee payment instructions, reconciled registers/pay statements, source-locked federal filing preparation data, quarter/year close controls, ledger posting, and reversal controls are implemented and tested. BrassLedger is not yet ready for live payroll until the remaining payroll goal is completed: configurable deduction/benefit plans and legal limits, bank-origin configuration and NACHA/check-file production, authoritative deposit schedules, transmitted federal/state/local forms and correction workflows, authoritative reciprocity and special-allocation content, all required state/local executable content, and independent accounting/tax review.
