...
Consolidation groups retain non-overlapping effective-dated ownership periods and require the operator to own every included company. Use **Map accounts** to assign each source account to an explicit effective-dated reporting identity and closing, average, or historical translation method; do not assume matching local account numbers have the same meaning. Maintain separately sourced rate types and configure a dedicated CTA equity reporting account in **Policy**. Historical reports use retained ownership, mapping, typed rates, posted ledger activity, and exact-period posted reporting adjustments. Missing rates or an unbalanced source-period selection stop CTA rather than silently substituting a rate or concealing an opening-balance problem. Prepare controlled manual adjustments and explicit company-pair eliminations under **Reporting**; automatic matching is not implied. See [Consolidation and currency reporting](consolidation-guide.md) for current calculations, duties, reversals, and limitations.
