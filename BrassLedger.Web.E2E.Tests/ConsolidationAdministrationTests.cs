    ...
    public async Task Reporting_PreparesApprovesPostsAndReversesAcquisitionSchedule(BrowserKind browserKind)
    {
        ...
        try
        {
            await using (var preparer = await fixture.CreateSessionAsync(browserKind))
            {
                ...
                await preparer.Page.Locator("#acquisitionMeasurementEnd").FillAsync("2027-08-31"); await preparer.Page.GetByLabel("Consideration component code").FillAsync("CASH"); await preparer.Page.GetByLabel("Consideration component description").FillAsync("Cash paid to sellers"); await preparer.Page.GetByLabel("Consideration component fair value").FillAsync("80"); await preparer.Page.GetByLabel("Consideration component source").FillAsync("E2E closing statement CS-1");
                await preparer.Page.GetByLabel("Identifiable item code").FillAsync("NET-ASSETS"); await preparer.Page.GetByLabel("Identifiable item description").FillAsync("Identifiable net assets"); await preparer.Page.GetByLabel("Identifiable item fair value").FillAsync("90"); await preparer.Page.GetByLabel("Identifiable item source").FillAsync("E2E valuation report VR-1");
