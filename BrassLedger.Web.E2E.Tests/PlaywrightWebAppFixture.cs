...
public sealed class PlaywrightWebAppFixture : IAsyncLifetime
{
    ...
    public async Task RemoveIntercompanyMatchingWorkflowAsync()
    {
        ...
        await using var command = connection.CreateCommand();
        command.CommandText = """
            DELETE FROM "BusinessAuditEntries" WHERE "Action" LIKE 'consolidation-intercompany-match.%' OR "Action" LIKE 'consolidation-trading-partner.%' OR "Action" LIKE 'consolidation-statement-presentation.%' OR "Action" LIKE 'consolidation-disclosure.%';
            DELETE FROM "ConsolidationIntercompanyMatches" WHERE "SalesInvoiceId" = '71000000-0000-0000-0000-000000000022';
