...
public sealed class PlaywrightWebAppFixture : IAsyncLifetime
{
    ...
    public async Task CreateConsolidationWorkflowAsync()
    {
        ...
        configuration.CommandText = """
            ...
            INSERT OR IGNORE INTO "SalesInvoices" ("Id", "CompanyId", "CustomerId", "InvoiceNumber", "InvoiceDate", "DueDate", "Status", "Subtotal", "TaxAmount", "TotalAmount", "BalanceDue", "TransactionCurrency", "TransactionSubtotal", "TransactionTaxAmount", "TransactionTotalAmount", "TransactionBalanceDue", "ExchangeRateToBase", "ExchangeRateEffectiveOn", "ExchangeRateSource", "ExchangeRateSourceReference", "SalesOrderId", "InventoryShipmentId", "ConcurrencyToken")
            SELECT '71000000-0000-0000-0000-000000000022', "Id", '71000000-0000-0000-0000-000000000020', 'E2E-IC-INV-1001', '2026-08-15', '2026-09-14', 'Open', '125.0', '0.0', '125.0', '125.0', 'USD', '125.0', '0.0', '125.0', '125.0', '1.0', '2026-08-15', 'Company base currency', '', NULL, NULL, 'e2e-ic-invoice-token'
            FROM "Companies" WHERE "Name" = 'Brass Ledger Manufacturing';
            INSERT OR IGNORE INTO "VendorBills" ("Id", "CompanyId", "VendorId", "BillNumber", "BillDate", "DueDate", "Status", "TotalAmount", "BalanceDue", "TransactionCurrency", "TransactionTotalAmount", "TransactionBalanceDue", "ExchangeRateToBase", "ExchangeRateEffectiveOn", "ExchangeRateSource", "ExchangeRateSourceReference", "PurchaseOrderId", "InventoryReceiptId", "ConcurrencyToken")
            VALUES ('71000000-0000-0000-0000-000000000023', '71000000-0000-0000-0000-000000000010', '71000000-0000-0000-0000-000000000021', 'e2e-ic-inv-1001', '2026-08-16', '2026-09-15', 'Open', '125.0', '125.0', 'USD', '125.0', '125.0', '1.0', '2026-08-16', 'Company base currency', '', NULL, NULL, 'e2e-ic-bill-token');
            """;
