...
public sealed class WorkspaceInitializationTests : IDisposable
{
    ...
    public async Task InitializeBrassLedgerAsync_CreatesSqliteDatabaseInAppData()
    {
        ...
        Assert.Equal("1", await ReadScalarAsync(connection, "SELECT COUNT(*) FROM __EFMigrationsHistory WHERE MigrationId = '20260827230231_AddTransactionCurrencyDocuments';"));
        Assert.Equal("1", await ReadScalarAsync(connection, "SELECT COUNT(*) FROM __EFMigrationsHistory WHERE MigrationId = '20260828014056_AddForeignCurrencyRemeasurements';"));
        Assert.Equal("1", await ReadScalarAsync(connection, "SELECT COUNT(*) FROM __EFMigrationsHistory WHERE MigrationId = '20260905034042_AddCustomerReturnCreditTransactionCurrency';"));
        Assert.Equal("1", await ReadScalarAsync(connection, "SELECT COUNT(*) FROM __EFMigrationsHistory WHERE MigrationId = '20260905221957_AddSupplierReturnShipmentTransactionCurrency';"));
        Assert.Equal("1", await ReadScalarAsync(connection, "SELECT COUNT(*) FROM sqlite_master WHERE type = 'table' AND name = 'ForeignCurrencyRemeasurementBatches';"));
        Assert.Equal("1", await ReadScalarAsync(connection, "SELECT COUNT(*) FROM sqlite_master WHERE type = 'table' AND name = 'ForeignCurrencyRemeasurementLines';"));
    ...
    public async Task InitializeBrassLedgerAsync_UpgradesPreLedgerDatabaseOnceAndRetainsData()
    {
        ...
        Assert.Equal("1", await ReadScalarAsync(verified, "SELECT COUNT(*) FROM __EFMigrationsHistory WHERE MigrationId = '20260827230231_AddTransactionCurrencyDocuments';"));
        Assert.Equal("1", await ReadScalarAsync(verified, "SELECT COUNT(*) FROM __EFMigrationsHistory WHERE MigrationId = '20260828014056_AddForeignCurrencyRemeasurements';"));
        Assert.Equal("1", await ReadScalarAsync(verified, "SELECT COUNT(*) FROM sqlite_master WHERE type = 'table' AND name = 'ForeignCurrencyRemeasurementBatches';"));
    ...
    [Fact]
    public async Task MigrationAdoption_RecognizesTransactionCurrencyAndRemeasurementSchemasWithoutReplayingThem()
    {
        using var services = CreateServiceProvider();
        await services.InitializeBrassLedgerAsync();
        var databasePath = Path.Combine(_contentRootPath, "App_Data", "brassledger.db");
        ...
        Assert.Equal("1", await ReadScalarAsync(verified, "SELECT COUNT(*) FROM __EFMigrationsHistory WHERE MigrationId = '20260827230231_AddTransactionCurrencyDocuments';"));
        Assert.Equal("1", await ReadScalarAsync(verified, "SELECT COUNT(*) FROM __EFMigrationsHistory WHERE MigrationId = '20260828014056_AddForeignCurrencyRemeasurements';"));
        Assert.Equal("1", await ReadScalarAsync(verified, "SELECT COUNT(*) FROM pragma_table_info('SalesInvoices') WHERE name = 'TransactionCurrency';"));
        Assert.Equal("1", await ReadScalarAsync(verified, "SELECT COUNT(*) FROM sqlite_master WHERE type = 'table' AND name = 'ForeignCurrencyRemeasurementLines';"));
        Assert.Equal("Brass Ledger Manufacturing", await ReadScalarAsync(verified, "SELECT Name FROM Companies WHERE Name = 'Brass Ledger Manufacturing';"));
    }

    [Fact]
    public async Task TransactionCurrencyMigration_BackfillsLegacyDocumentsFromCompanyBaseCurrency()
    {
        using var services = CreateServiceProvider(); using var scope = services.CreateScope();
        ...
        var migrator = db.Database.GetService<IMigrator>();
        await migrator.MigrateAsync("20260827201050_AddConsolidationOwnershipEvents");
        var companyId = Guid.NewGuid(); var customerId = Guid.NewGuid(); var vendorId = Guid.NewGuid(); var invoiceId = Guid.NewGuid(); var billId = Guid.NewGuid();
        await db.Database.ExecuteSqlInterpolatedAsync($"""INSERT INTO "Companies" ("Id", "Name", "LegalName", "TaxId", "BaseCurrency", "FiscalYearStartMonth") VALUES ({companyId}, {"CAD legacy company"}, {"CAD legacy company"}, {"00-0000000"}, {"CAD"}, {1});""");
        await db.Database.ExecuteSqlInterpolatedAsync($"""INSERT INTO "Customers" ("Id", "CompanyId", "CustomerNumber", "Name", "Email", "State", "CreditLimit", "OpenBalance") VALUES ({customerId}, {companyId}, {"C-LEGACY-FX"}, {"Legacy customer"}, {"customer@example.test"}, {"ON"}, {1000m}, {80m});""");
        await db.Database.ExecuteSqlInterpolatedAsync($"""INSERT INTO "Vendors" ("Id", "CompanyId", "VendorNumber", "Name", "Email", "State", "PaymentTerms", "OpenBalance") VALUES ({vendorId}, {companyId}, {"V-LEGACY-FX"}, {"Legacy vendor"}, {"vendor@example.test"}, {"ON"}, {"Net 30"}, {60m});""");
        await db.Database.ExecuteSqlInterpolatedAsync($"""INSERT INTO "SalesInvoices" ("Id", "CompanyId", "CustomerId", "InvoiceNumber", "InvoiceDate", "DueDate", "Status", "Subtotal", "TaxAmount", "TotalAmount", "BalanceDue", "ConcurrencyToken") VALUES ({invoiceId}, {companyId}, {customerId}, {"INV-LEGACY-FX"}, {new DateOnly(2026, 1, 1)}, {new DateOnly(2026, 1, 31)}, {"Partial"}, {100m}, {5m}, {105m}, {80m}, {"legacy-invoice"});""");
        await db.Database.ExecuteSqlInterpolatedAsync($"""INSERT INTO "VendorBills" ("Id", "CompanyId", "VendorId", "BillNumber", "BillDate", "DueDate", "Status", "TotalAmount", "BalanceDue", "ConcurrencyToken") VALUES ({billId}, {companyId}, {vendorId}, {"B-LEGACY-FX"}, {new DateOnly(2026, 1, 2)}, {new DateOnly(2026, 2, 1)}, {"Partial"}, {90m}, {60m}, {"legacy-bill"});""");

        await migrator.MigrateAsync(); db.ChangeTracker.Clear();

        var invoice = await db.SalesInvoices.SingleAsync(item => item.Id == invoiceId); var bill = await db.VendorBills.SingleAsync(item => item.Id == billId);
        Assert.Equal("CAD", invoice.TransactionCurrency); Assert.Equal(105m, invoice.TransactionTotalAmount); Assert.Equal(80m, invoice.TransactionBalanceDue); Assert.Equal(1m, invoice.ExchangeRateToBase); Assert.Null(invoice.ExchangeRateId);
        Assert.Equal("CAD", bill.TransactionCurrency); Assert.Equal(90m, bill.TransactionTotalAmount); Assert.Equal(60m, bill.TransactionBalanceDue); Assert.Equal(1m, bill.ExchangeRateToBase); Assert.Null(bill.ExchangeRateId);
    }
    ...
    [Fact]
    public async Task ForeignCurrencyDocuments_FreezeRatesAndRecognizeSettlementGainLoss()
    {
        using var services = CreateServiceProvider();
        await services.InitializeBrassLedgerAsync();
        using var scope = services.CreateScope();
        ...
        var customer = initial.Receivables.Customers.First(); var vendor = initial.Payables.Vendors.First(); var bank = initial.Treasury.BankAccounts.First();
        var documentRateId = Guid.NewGuid(); var settlementRateId = Guid.NewGuid(); var inverseRateId = Guid.NewGuid(); var futureRateId = Guid.NewGuid();
        await using (var db = await factory.CreateDbContextAsync())
        {
            var companyId = await db.Companies.Select(company => company.Id).SingleAsync();
            db.CurrencyExchangeRates.AddRange(
                new() { Id = documentRateId, CompanyId = companyId, BaseCurrency = "CAD", QuoteCurrency = "USD", Rate = .75m, RateType = CurrencyRateType.Closing, EffectiveOn = new DateOnly(2026, 5, 1), Source = "Bank of Canada daily rate", SourceReference = "https://example.test/cad-usd/2026-05-01", RetrievedOn = new DateOnly(2026, 5, 2) },
                new() { Id = settlementRateId, CompanyId = companyId, BaseCurrency = "CAD", QuoteCurrency = "USD", Rate = .80m, RateType = CurrencyRateType.Closing, EffectiveOn = new DateOnly(2026, 5, 2), Source = "Bank of Canada daily rate", SourceReference = "https://example.test/cad-usd/2026-05-02", RetrievedOn = new DateOnly(2026, 5, 3) },
                new() { Id = inverseRateId, CompanyId = companyId, BaseCurrency = "USD", QuoteCurrency = "EUR", Rate = .50m, RateType = CurrencyRateType.Closing, EffectiveOn = new DateOnly(2026, 5, 1), Source = "ECB inverse quotation", SourceReference = "https://example.test/usd-eur/2026-05-01" },
                new() { Id = futureRateId, CompanyId = companyId, BaseCurrency = "CAD", QuoteCurrency = "USD", Rate = .85m, RateType = CurrencyRateType.Closing, EffectiveOn = new DateOnly(2026, 5, 10), Source = "Future observation", SourceReference = "https://example.test/cad-usd/2026-05-10" });
            await db.SaveChangesAsync();
        }

        var invoice = await PostInvoiceThroughWorkflowAsync(transactions, new(customer.Id, "INV-FX-CAD-1", new DateOnly(2026, 5, 1), new DateOnly(2026, 5, 31), 100m, 0m, "4000", "CAD consulting invoice", Currency: "CAD", ExchangeRateId: documentRateId));
        var bill = await PostVendorBillThroughWorkflowAsync(transactions, new(vendor.Id, "B-FX-CAD-1", new DateOnly(2026, 5, 1), new DateOnly(2026, 5, 31), 100m, "5100", "CAD professional-services bill", Currency: "CAD", ExchangeRateId: documentRateId));
        Assert.True(invoice.Succeeded, invoice.ErrorMessage); Assert.True(bill.Succeeded, bill.ErrorMessage);
        var inverseInvoice = await PostInvoiceThroughWorkflowAsync(transactions, new(customer.Id, "INV-FX-EUR-1", new DateOnly(2026, 5, 1), new DateOnly(2026, 5, 31), 10m, 0m, "4000", "EUR invoice using inverse quote", Currency: "EUR", ExchangeRateId: inverseRateId));
        Assert.True(inverseInvoice.Succeeded, inverseInvoice.ErrorMessage);
        Assert.Equal(20m, (await workspaceService.GetWorkspaceAsync()).Receivables.Invoices.Single(item => item.Id == inverseInvoice.Id).TotalAmount);
        Assert.False((await PostInvoiceThroughWorkflowAsync(transactions, new(customer.Id, "INV-FX-FUTURE", new DateOnly(2026, 5, 1), new DateOnly(2026, 5, 31), 10m, 0m, "4000", "Future rate rejected", Currency: "CAD", ExchangeRateId: futureRateId))).Succeeded);
        ...
        var disbursement = await transactions.RecordVendorPaymentAsync(new(vendor.Id, bank.Id, new DateOnly(2026, 5, 2), 100m, "PAY-FX-CAD-1", "Wire", [new(bill.Id!.Value, 100m)], "CAD", settlementRateId));
        Assert.True(receipt.Succeeded, receipt.ErrorMessage); Assert.True(finalReceipt.Succeeded, finalReceipt.ErrorMessage); Assert.True(disbursement.Succeeded, disbursement.ErrorMessage);
        await using (var db = await factory.CreateDbContextAsync())
        {
            var customerPayment = await db.SubledgerPayments.SingleAsync(item => item.Id == receipt.Id); var finalCustomerPayment = await db.SubledgerPayments.SingleAsync(item => item.Id == finalReceipt.Id); var vendorPayment = await db.SubledgerPayments.SingleAsync(item => item.Id == disbursement.Id);
            Assert.Equal(32m, customerPayment.Amount); Assert.Equal(30m, customerPayment.AppliedAmount); Assert.Equal(2m, customerPayment.RealizedGainLoss); Assert.Equal(40m, customerPayment.TransactionAmount); Assert.Equal(.80m, customerPayment.ExchangeRateToBase);
            Assert.Equal(48m, finalCustomerPayment.Amount); Assert.Equal(45m, finalCustomerPayment.AppliedAmount); Assert.Equal(3m, finalCustomerPayment.RealizedGainLoss); Assert.Equal(60m, finalCustomerPayment.TransactionAmount);
            Assert.Equal(80m, vendorPayment.Amount); Assert.Equal(75m, vendorPayment.AppliedAmount); Assert.Equal(-5m, vendorPayment.RealizedGainLoss);
            var customerGainLines = await db.JournalEntryLines.Where(line => line.JournalEntryId == customerPayment.JournalEntryId).Join(db.Accounts, line => line.AccountId, account => account.Id, (line, account) => new { line, account }).Where(item => item.account.OperationalRole == AccountingAccountRoles.ForeignExchangeGain).Select(item => item.line.Credit).ToListAsync();
            var vendorLossLines = await db.JournalEntryLines.Where(line => line.JournalEntryId == vendorPayment.JournalEntryId).Join(db.Accounts, line => line.AccountId, account => account.Id, (line, account) => new { line, account }).Where(item => item.account.OperationalRole == AccountingAccountRoles.ForeignExchangeLoss).Select(item => item.line.Debit).ToListAsync();
            Assert.Equal(2m, customerGainLines.Sum());
            Assert.Equal(5m, vendorLossLines.Sum());
        }
        var settled = await workspaceService.GetWorkspaceAsync();
        Assert.Equal(0m, settled.Receivables.Invoices.Single(item => item.Id == invoice.Id).TransactionBalanceDue); Assert.Equal(0m, settled.Payables.Bills.Single(item => item.Id == bill.Id).TransactionBalanceDue);
        ...
        Assert.Equal(100m, restored.Receivables.Invoices.Single(item => item.Id == invoice.Id).TransactionBalanceDue); Assert.Equal(75m, restored.Receivables.Invoices.Single(item => item.Id == invoice.Id).BalanceDue);
        Assert.Equal(100m, restored.Payables.Bills.Single(item => item.Id == bill.Id).TransactionBalanceDue); Assert.Equal(75m, restored.Payables.Bills.Single(item => item.Id == bill.Id).BalanceDue);
        await using (var db = await factory.CreateDbContextAsync())
        {
            var reversedReceipt = await db.SubledgerPayments.SingleAsync(item => item.Id == receipt.Id);
            Assert.Equal(new DateOnly(2026, 5, 3), reversedReceipt.ReversalDate);
            Assert.True(await db.SubledgerPaymentApplications.AnyAsync(item => item.SubledgerPaymentId == receipt.Id && item.DocumentId == invoice.Id));
        }
        var retrospectiveRemeasurement = await transactions.PrepareForeignCurrencyRemeasurementAsync(new(new DateOnly(2026, 5, 1), "FX-HISTORICAL-AFTER-REVERSALS", [new("CAD", documentRateId), new("EUR", inverseRateId)]));
        Assert.False(retrospectiveRemeasurement.Succeeded); Assert.Contains("activity after", retrospectiveRemeasurement.ErrorMessage, StringComparison.OrdinalIgnoreCase);
    }

    [Fact]
    public async Task ForeignCurrencyRemeasurement_RetainsReviewPostsUnrealizedResultsAndReversesExactly()
    {
        using var services = CreateServiceProvider(); await services.InitializeBrassLedgerAsync(); using var scope = services.CreateScope();
        var transactions = scope.ServiceProvider.GetRequiredService<IAccountingTransactionService>(); var workspaceService = scope.ServiceProvider.GetRequiredService<IBusinessWorkspaceService>(); var accountingPeriods = scope.ServiceProvider.GetRequiredService<IAccountingPeriodService>(); var factory = scope.ServiceProvider.GetRequiredService<IDbContextFactory<BrassLedgerDbContext>>(); var accessor = scope.ServiceProvider.GetRequiredService<Microsoft.AspNetCore.Http.IHttpContextAccessor>();
        var initial = await workspaceService.GetWorkspaceAsync(); var customer = initial.Receivables.Customers.First(); var vendor = initial.Payables.Vendors.First(); var documentRateId = Guid.NewGuid(); var closingRateId = Guid.NewGuid(); Guid companyId;
        await using (var db = await factory.CreateDbContextAsync())
        {
            companyId = await db.Companies.Select(company => company.Id).SingleAsync();
            db.CurrencyExchangeRates.AddRange(
                new() { Id = documentRateId, CompanyId = companyId, BaseCurrency = "CAD", QuoteCurrency = "USD", Rate = .75m, RateType = CurrencyRateType.Closing, EffectiveOn = new DateOnly(2026, 5, 1), Source = "Official daily rate", SourceReference = "https://example.test/2026-05-01" },
                new() { Id = closingRateId, CompanyId = companyId, BaseCurrency = "CAD", QuoteCurrency = "USD", Rate = .80m, RateType = CurrencyRateType.Closing, EffectiveOn = new DateOnly(2026, 5, 31), Source = "Official month-end rate", SourceReference = "https://example.test/2026-05-31" });
            await db.SaveChangesAsync();
        }
        var invoice = await PostInvoiceThroughWorkflowAsync(transactions, new(customer.Id, "INV-FX-REMEASURE-1", new DateOnly(2026, 5, 1), new DateOnly(2026, 6, 1), 100m, 0m, "4000", "Open CAD receivable", Currency: "CAD", ExchangeRateId: documentRateId));
        var bill = await PostVendorBillThroughWorkflowAsync(transactions, new(vendor.Id, "B-FX-REMEASURE-1", new DateOnly(2026, 5, 1), new DateOnly(2026, 6, 1), 100m, "5100", "Open CAD payable", Currency: "CAD", ExchangeRateId: documentRateId));
        Assert.True(invoice.Succeeded, invoice.ErrorMessage); Assert.True(bill.Succeeded, bill.ErrorMessage);
        void ActAs(Guid userId, params string[] permissions)
        {
            var claims = new List<System.Security.Claims.Claim> { new(BrassLedgerAuthenticationDefaults.CompanyIdClaimType, companyId.ToString()), new(System.Security.Claims.ClaimTypes.NameIdentifier, userId.ToString()) };
        ...
        var preparer = Guid.NewGuid(); var reviewer = Guid.NewGuid(); var poster = Guid.NewGuid(); var reverser = Guid.NewGuid();
        ActAs(preparer, BrassLedgerPermissions.JournalPrepare, BrassLedgerPermissions.JournalApprove);
        var period = await accountingPeriods.SavePeriodAsync(new(null, new DateOnly(2026, 5, 1), new DateOnly(2026, 5, 31), "Month-end FX control")); Assert.True(period.Succeeded, period.ErrorMessage);
        var prematureClose = await accountingPeriods.SetPeriodStatusAsync(period.Id!.Value, true, "Attempt close before FX review"); Assert.False(prematureClose.Succeeded); Assert.Contains("remeasurement", prematureClose.ErrorMessage, StringComparison.OrdinalIgnoreCase);
        var prepared = await transactions.PrepareForeignCurrencyRemeasurementAsync(new(new DateOnly(2026, 5, 31), "FX-2026-05", [new("CAD", closingRateId)])); Assert.True(prepared.Succeeded, prepared.ErrorMessage);
        var batch = (await transactions.GetForeignCurrencyRemeasurementsAsync()).Single(item => item.Id == prepared.Id); Assert.Equal("Draft", batch.Status); Assert.Equal(2, batch.Lines.Count); Assert.Equal(0m, batch.NetAdjustment); Assert.All(batch.Lines, line => { Assert.Equal(100m, line.TransactionBalance); Assert.Equal(75m, line.PreviousBaseBalance); Assert.Equal(80m, line.RemeasuredBaseBalance); Assert.Equal(5m, line.AdjustmentAmount); Assert.Equal(.80m, line.ExchangeRateToBase); Assert.Contains("2026-05-31", line.ExchangeRateSourceReference); });
        var selfReview = await transactions.DecideForeignCurrencyRemeasurementAsync(new(batch.Id, true, "", batch.ConcurrencyToken)); Assert.False(selfReview.Succeeded); Assert.Contains("prepared", selfReview.ErrorMessage, StringComparison.OrdinalIgnoreCase);
        ActAs(reviewer, BrassLedgerPermissions.JournalApprove, BrassLedgerPermissions.JournalPost); batch = (await transactions.GetForeignCurrencyRemeasurementsAsync()).Single(item => item.Id == prepared.Id); Assert.True((await transactions.DecideForeignCurrencyRemeasurementAsync(new(batch.Id, true, "Reviewed month-end rates.", batch.ConcurrencyToken))).Succeeded);
        batch = (await transactions.GetForeignCurrencyRemeasurementsAsync()).Single(item => item.Id == prepared.Id); var selfPost = await transactions.PostForeignCurrencyRemeasurementAsync(new(batch.Id, batch.ConcurrencyToken)); Assert.False(selfPost.Succeeded); Assert.Contains("approved", selfPost.ErrorMessage, StringComparison.OrdinalIgnoreCase);
        ActAs(poster, BrassLedgerPermissions.JournalPost); Assert.False((await transactions.PostForeignCurrencyRemeasurementAsync(new(batch.Id, "stale"))).Succeeded); Assert.True((await transactions.PostForeignCurrencyRemeasurementAsync(new(batch.Id, batch.ConcurrencyToken))).Succeeded);
        var remeasured = await workspaceService.GetWorkspaceAsync(); Assert.Equal(80m, remeasured.Receivables.Invoices.Single(item => item.Id == invoice.Id).BalanceDue); Assert.Equal(100m, remeasured.Receivables.Invoices.Single(item => item.Id == invoice.Id).TransactionBalanceDue); Assert.Equal(80m, remeasured.Payables.Bills.Single(item => item.Id == bill.Id).BalanceDue); Assert.Equal(100m, remeasured.Payables.Bills.Single(item => item.Id == bill.Id).TransactionBalanceDue);
        batch = (await transactions.GetForeignCurrencyRemeasurementsAsync()).Single(item => item.Id == prepared.Id); Assert.Equal("Posted", batch.Status); Assert.NotNull(batch.JournalEntryId);
        await using (var db = await factory.CreateDbContextAsync())
        {
            var fxLines = await db.JournalEntryLines.Where(line => line.JournalEntryId == batch.JournalEntryId).Join(db.Accounts, line => line.AccountId, account => account.Id, (line, account) => new { line, account.OperationalRole }).ToListAsync();
            Assert.Equal(5m, fxLines.Where(item => item.OperationalRole == AccountingAccountRoles.ForeignExchangeGain).Sum(item => item.line.Credit)); Assert.Equal(5m, fxLines.Where(item => item.OperationalRole == AccountingAccountRoles.ForeignExchangeLoss).Sum(item => item.line.Debit));
        }
        ActAs(preparer, BrassLedgerPermissions.JournalPrepare); var retrospectiveBatch = await transactions.PrepareForeignCurrencyRemeasurementAsync(new(new DateOnly(2026, 5, 30), "FX-2026-05-30-LATE", [new("CAD", documentRateId)])); Assert.False(retrospectiveBatch.Succeeded); Assert.Contains("later", retrospectiveBatch.ErrorMessage, StringComparison.OrdinalIgnoreCase);
        await using (var db = await factory.CreateDbContextAsync())
        {
            var changedInvoice = await db.SalesInvoices.SingleAsync(item => item.Id == invoice.Id);
            changedInvoice.TransactionBalanceDue = 0m; changedInvoice.BalanceDue = 0m; changedInvoice.Status = "Paid";
            await db.SaveChangesAsync();
        }
        ActAs(poster, BrassLedgerPermissions.JournalPost); var changedBalanceClose = await accountingPeriods.SetPeriodStatusAsync(period.Id.Value, true, "Attempt close after document activity"); Assert.False(changedBalanceClose.Succeeded); Assert.Contains("changed", changedBalanceClose.ErrorMessage, StringComparison.OrdinalIgnoreCase);
        await using (var db = await factory.CreateDbContextAsync())
        {
            var changedInvoice = await db.SalesInvoices.SingleAsync(item => item.Id == invoice.Id);
            changedInvoice.TransactionBalanceDue = 100m; changedInvoice.BalanceDue = 80m; changedInvoice.Status = "Open";
            await db.SaveChangesAsync();
        }
        ActAs(poster, BrassLedgerPermissions.JournalPost); Assert.True((await accountingPeriods.SetPeriodStatusAsync(period.Id.Value, true, "Closed after reviewed FX remeasurement")).Succeeded);
        ActAs(reverser, BrassLedgerPermissions.JournalReverse); var closedReversal = await transactions.ReverseForeignCurrencyRemeasurementAsync(new(batch.Id, new DateOnly(2026, 6, 1), "Correct the retained closing rate.", batch.ConcurrencyToken)); Assert.False(closedReversal.Succeeded); Assert.Contains("Reopen", closedReversal.ErrorMessage, StringComparison.OrdinalIgnoreCase);
        Assert.True((await accountingPeriods.SetPeriodStatusAsync(period.Id.Value, false, "Reopened to correct FX rate")).Succeeded);
        ActAs(preparer, BrassLedgerPermissions.JournalPrepare); var laterPrepared = await transactions.PrepareForeignCurrencyRemeasurementAsync(new(new DateOnly(2026, 6, 30), "FX-2026-06", [new("CAD", closingRateId)])); Assert.True(laterPrepared.Succeeded, laterPrepared.ErrorMessage);
        var laterBatch = (await transactions.GetForeignCurrencyRemeasurementsAsync()).Single(item => item.Id == laterPrepared.Id);
        ActAs(reviewer, BrassLedgerPermissions.JournalApprove); Assert.True((await transactions.DecideForeignCurrencyRemeasurementAsync(new(laterBatch.Id, true, "Reviewed unchanged June rate.", laterBatch.ConcurrencyToken))).Succeeded);
        laterBatch = (await transactions.GetForeignCurrencyRemeasurementsAsync()).Single(item => item.Id == laterPrepared.Id);
        ActAs(poster, BrassLedgerPermissions.JournalPost); Assert.True((await transactions.PostForeignCurrencyRemeasurementAsync(new(laterBatch.Id, laterBatch.ConcurrencyToken))).Succeeded);
        laterBatch = (await transactions.GetForeignCurrencyRemeasurementsAsync()).Single(item => item.Id == laterPrepared.Id); Assert.Null(laterBatch.JournalEntryId);
        ActAs(reverser, BrassLedgerPermissions.JournalReverse); var outOfOrderReversal = await transactions.ReverseForeignCurrencyRemeasurementAsync(new(batch.Id, new DateOnly(2026, 7, 1), "Out-of-order reversal must fail.", batch.ConcurrencyToken)); Assert.False(outOfOrderReversal.Succeeded); Assert.Contains("later", outOfOrderReversal.ErrorMessage, StringComparison.OrdinalIgnoreCase);
        Assert.True((await transactions.ReverseForeignCurrencyRemeasurementAsync(new(laterBatch.Id, new DateOnly(2026, 7, 1), "Reverse later unchanged batch first.", laterBatch.ConcurrencyToken))).Succeeded);
        var chronologicallyInvalidReversal = await transactions.ReverseForeignCurrencyRemeasurementAsync(new(batch.Id, new DateOnly(2026, 6, 1), "Cannot predate the later batch reversal.", batch.ConcurrencyToken)); Assert.False(chronologicallyInvalidReversal.Succeeded); Assert.Contains("cannot precede", chronologicallyInvalidReversal.ErrorMessage, StringComparison.OrdinalIgnoreCase);
        Assert.True((await transactions.ReverseForeignCurrencyRemeasurementAsync(new(batch.Id, new DateOnly(2026, 7, 1), "Correct the retained closing rate.", batch.ConcurrencyToken))).Succeeded);
        var restored = await workspaceService.GetWorkspaceAsync(); Assert.Equal(75m, restored.Receivables.Invoices.Single(item => item.Id == invoice.Id).BalanceDue); Assert.Equal(75m, restored.Payables.Bills.Single(item => item.Id == bill.Id).BalanceDue);
        batch = (await transactions.GetForeignCurrencyRemeasurementsAsync()).Single(item => item.Id == prepared.Id); Assert.Equal("Reversed", batch.Status); Assert.Equal(new DateOnly(2026, 7, 1), batch.ReversalDate); Assert.NotNull(batch.ReversalJournalEntryId); Assert.Equal("Correct the retained closing rate.", batch.ReversalReason);
    }
    ...
    [Fact]
    public async Task ForeignCustomerReturnCredit_TranslatesAtOriginalInvoiceRateAndAppliesAndReversesExactly()
    {
        // The SalesOrder -> InventoryShipment -> InvoiceInventoryShipmentAsync pipeline that
        // CustomerReturnCredit's authorization/receipt chain is built on has no transaction-currency
        // support at all (SalesOrder/SalesOrderLine/InvoiceInventoryShipmentRequest carry no currency
        // or rate fields), unlike the standalone itemized-invoice path PostInvoiceThroughWorkflowAsync
        // uses. So this test drives the real shipment/return-authorization/return-receipt chain through
        // the real service (base-currency, since that part only tracks inventory cost, not sale price),
        // then seeds the foreign SalesInvoice/SalesInvoiceLine directly -- exactly mirroring the
        // Base/Transaction field population AccountingTransactionService.cs's direct-invoice path uses
        // -- since no service method exists yet to create a shipment-linked foreign invoice. That gap is
        // a real, separate prerequisite for a fully end-to-end foreign customer-return workflow; this
        // test instead proves CreditCustomerReturnAsync/ApplyCustomerReturnCreditAsync's own FX
        // translation, application, and exact reversal against a realistic foreign invoice line shape.
        using var services = CreateServiceProvider(); await services.InitializeBrassLedgerAsync(); using var scope = services.CreateScope();
        var transactions = scope.ServiceProvider.GetRequiredService<IAccountingTransactionService>(); var factory = scope.ServiceProvider.GetRequiredService<IDbContextFactory<BrassLedgerDbContext>>();
        Guid customerId; Guid itemId; Guid companyId; Guid revenueAccountId; decimal startingCustomerBalance; decimal startingReceivables;
        await using (var db = await factory.CreateDbContextAsync())
        {
        ...
        var documentRateId = Guid.NewGuid();
        await using (var db = await factory.CreateDbContextAsync())
        {
            db.CurrencyExchangeRates.Add(new() { Id = documentRateId, CompanyId = companyId, BaseCurrency = "CAD", QuoteCurrency = "USD", Rate = .75m, RateType = CurrencyRateType.Closing, EffectiveOn = new DateOnly(2026, 6, 1), Source = "Test document rate", SourceReference = "https://example.test/cad-usd/2026-06-01" });
            await db.SaveChangesAsync();
        }

        // Real base-currency order/allocate/ship pipeline -- only inventory cost flows through this, no sale price.
        var orderResult = await transactions.SaveSalesOrderAsync(new(null, customerId, "SO-FXRETURN-1", new DateOnly(2026, 6, 1), null, "Foreign return provenance test", [new SalesOrderLineRequest(itemId, "Foreign compression kits", 2m, 50m, 0m, 8m, "4000")])); Assert.True(orderResult.Succeeded, orderResult.ErrorMessage);
        ...
        var shipmentResult = await transactions.ShipSalesOrderAsync(new(orderResult.Id!.Value, "SHIP-FXRETURN-1", new DateOnly(2026, 6, 1), [new(orderLineId, 2m)], orderToken)); Assert.True(shipmentResult.Succeeded, shipmentResult.ErrorMessage);
        ...
        // The invoice is already fully settled (BalanceDue/TransactionBalanceDue = 0), matching the base-
        // currency test's pattern, so the whole return credit remains available for a separate application
        // below rather than auto-applying against this invoice's own (already-zero) balance.
        ...
        var authorizationResult = await transactions.AuthorizeCustomerReturnAsync(new(shipmentResult.Id!.Value, "RMA-FXRETURN-1", new DateOnly(2026, 6, 2), "Customer changed requirements", [new(shipmentLineId, 1m)], shipmentToken)); Assert.True(authorizationResult.Succeeded, authorizationResult.ErrorMessage);
        string authorizationToken; Guid authorizationLineId; await using (var db = await factory.CreateDbContextAsync()) { var authorization = await db.CustomerReturnAuthorizations.SingleAsync(x => x.Id == authorizationResult.Id); authorizationToken = authorization.ConcurrencyToken; authorizationLineId = await db.CustomerReturnAuthorizationLines.Where(x => x.CustomerReturnAuthorizationId == authorization.Id).Select(x => x.Id).SingleAsync(); }
        var receiptResult = await transactions.ReceiveCustomerReturnAsync(new(authorizationResult.Id!.Value, "CRCV-FXRETURN-1", new DateOnly(2026, 6, 3), null, null, [new(authorizationLineId, 1m)], authorizationToken)); Assert.True(receiptResult.Succeeded, receiptResult.ErrorMessage);
        ...
        // Returning 1 of 2 units: proportional (not final) split of both the transaction- and base-currency amounts.
        var creditResult = await transactions.CreditCustomerReturnAsync(new(receiptResult.Id!.Value, "CM-FXRETURN-1", new DateOnly(2026, 6, 4), "Foreign return accepted", receiptToken)); Assert.True(creditResult.Succeeded, creditResult.ErrorMessage);
        string creditToken; await using (var db = await factory.CreateDbContextAsync())
        {
            var credit = await db.CustomerReturnCredits.SingleAsync(x => x.Id == creditResult.Id); creditToken = credit.ConcurrencyToken;
            Assert.Equal(50m, credit.TransactionSubtotal); Assert.Equal(4m, credit.TransactionTaxAmount); Assert.Equal(54m, credit.TransactionTotalAmount); Assert.Equal("CAD", credit.TransactionCurrency); Assert.Equal(.75m, credit.ExchangeRateToBase); Assert.Equal(documentRateId, credit.ExchangeRateId);
            Assert.Equal(37.5m, credit.Subtotal); Assert.Equal(3m, credit.TaxAmount); Assert.Equal(40.5m, credit.TotalAmount);
            Assert.Equal(0m, credit.SourceAppliedAmount); Assert.Equal(0m, credit.TransactionSourceAppliedAmount); Assert.Equal(0m, credit.AppliedAmount); Assert.Equal(0m, credit.TransactionAppliedAmount);
            Assert.Equal(startingReceivables - 40.5m, await db.Accounts.Where(x => x.OperationalRole == AccountingAccountRoles.AccountsReceivable).Select(x => x.CurrentBalance).SingleAsync());
            Assert.Equal(startingCustomerBalance - 40.5m, await db.Customers.Where(x => x.Id == customerId).Select(x => x.OpenBalance).SingleAsync());
        }

        // A second, unrelated foreign invoice for the same customer, at the same rate, to apply the credit to.
        var otherInvoiceId = Guid.NewGuid();
        await using (var db = await factory.CreateDbContextAsync())
        {
            db.SalesInvoices.Add(new SalesInvoice { Id = otherInvoiceId, CompanyId = companyId, CustomerId = customerId, InvoiceNumber = "INV-FXRETURN-OTHER", InvoiceDate = new DateOnly(2026, 6, 1), DueDate = new DateOnly(2026, 7, 1), Status = "Open", Subtotal = 40m, TaxAmount = 5m, TotalAmount = 45m, BalanceDue = 45m, TransactionCurrency = "CAD", TransactionSubtotal = 53.33m, TransactionTaxAmount = 6.67m, TransactionTotalAmount = 60m, TransactionBalanceDue = 60m, ExchangeRateId = documentRateId, ExchangeRateToBase = .75m, ExchangeRateEffectiveOn = new DateOnly(2026, 6, 1), ExchangeRateSource = "Test document rate", ConcurrencyToken = Guid.NewGuid().ToString("N") });
            await db.SaveChangesAsync(); // hand-inserted like the first invoice, so it never touched customer.OpenBalance -- only its own BalanceDue/TransactionBalanceDue matter for the application below.
        }

        // Applying more than the available transaction-currency credit fails.
        var overApplication = await transactions.ApplyCustomerReturnCreditAsync(new(creditResult.Id!.Value, otherInvoiceId, new DateOnly(2026, 6, 5), 54.01m, creditToken)); Assert.False(overApplication.Succeeded); Assert.Contains("exceeds", overApplication.ErrorMessage, StringComparison.OrdinalIgnoreCase);

        var applicationResult = await transactions.ApplyCustomerReturnCreditAsync(new(creditResult.Id!.Value, otherInvoiceId, new DateOnly(2026, 6, 5), 54m, creditToken)); Assert.True(applicationResult.Succeeded, applicationResult.ErrorMessage);
        string applicationToken; await using (var db = await factory.CreateDbContextAsync())
        {
            var application = await db.CustomerReturnCreditApplications.SingleAsync(x => x.Id == applicationResult.Id); applicationToken = application.ConcurrencyToken;
            Assert.Equal(54m, application.TransactionAmount); Assert.Equal(40.5m, application.Amount);
            var credit = await db.CustomerReturnCredits.SingleAsync(x => x.Id == creditResult.Id); Assert.Equal(54m, credit.TransactionAppliedAmount); Assert.Equal(40.5m, credit.AppliedAmount);
            var otherInvoice = await db.SalesInvoices.SingleAsync(x => x.Id == otherInvoiceId); Assert.Equal(6m, otherInvoice.TransactionBalanceDue); Assert.Equal(4.5m, otherInvoice.BalanceDue);
        }

        // Exact reversal, newest to oldest: the application, then the credit itself.
        Assert.True((await transactions.ReverseCustomerReturnCreditApplicationAsync(new(applicationResult.Id!.Value, new DateOnly(2026, 6, 6), "Apply to a different document instead", applicationToken))).Succeeded);
        await using (var db = await factory.CreateDbContextAsync())
        {
            var credit = await db.CustomerReturnCredits.SingleAsync(x => x.Id == creditResult.Id); Assert.Equal(0m, credit.TransactionAppliedAmount); Assert.Equal(0m, credit.AppliedAmount); creditToken = credit.ConcurrencyToken;
            var otherInvoice = await db.SalesInvoices.SingleAsync(x => x.Id == otherInvoiceId); Assert.Equal(60m, otherInvoice.TransactionBalanceDue); Assert.Equal(45m, otherInvoice.BalanceDue);
        }
        Assert.True((await transactions.ReverseCustomerReturnCreditAsync(new(creditResult.Id!.Value, new DateOnly(2026, 6, 6), "Return rejected after inspection", creditToken))).Succeeded);
        ...
        Assert.Equal(0m, await afterFx.SalesInvoices.Where(x => x.Id == invoiceId).Select(x => x.BalanceDue).SingleAsync()); Assert.Equal(0m, await afterFx.SalesInvoices.Where(x => x.Id == invoiceId).Select(x => x.TransactionBalanceDue).SingleAsync());
    ...
    [Fact]
    public async Task ForeignSupplierReturnCredit_TranslatesAtOriginalBillRateAndAppliesAndReversesExactly()
    {
        // The PurchaseOrder -> InventoryReceipt -> SavePurchaseInvoiceMatchAsync pipeline that
        // SupplierReturnShipment's authorization/receipt chain is built on has no transaction-currency
        // support at all (PurchaseOrderLine/ReceivePurchaseOrderRequest/PurchaseInvoiceMatchLineRequest
        // carry no currency or rate fields). So this test drives the real order/receipt/return-
        // authorization chain through the real service (base-currency, since that part only tracks
        // inventory cost, not the vendor's own invoiced price), then seeds the foreign VendorBill/
        // VendorBillLine directly -- exactly mirroring the Base/Transaction field population
        // AccountingTransactionService.PurchaseInvoiceMatches.cs's real posting path uses -- since no
        // service method exists yet to create a receipt-matched foreign bill. That gap is a real,
        // separate prerequisite for a fully end-to-end foreign supplier-return workflow; this test
        // instead proves ShipSupplierReturnAsync/ApplySupplierReturnCreditAsync's own FX translation,
        // application, and exact reversal (including the source-bill TransactionBalanceDue restoration
        // on shipment reversal) against a realistic foreign vendor-bill line shape.
        using var services = CreateServiceProvider(); await services.InitializeBrassLedgerAsync(); using var scope = services.CreateScope();
        var transactions = scope.ServiceProvider.GetRequiredService<IAccountingTransactionService>(); var factory = scope.ServiceProvider.GetRequiredService<IDbContextFactory<BrassLedgerDbContext>>();
        Guid companyId; Guid vendorId; Guid itemId; Guid grniAccountId; decimal startingVendorBalance; decimal startingPayables;
        await using (var db = await factory.CreateDbContextAsync())
        {
            companyId = await db.Companies.Select(x => x.Id).SingleAsync();
        ...
        await using (var db = await factory.CreateDbContextAsync())
        {
            db.CurrencyExchangeRates.Add(new() { Id = documentRateId, CompanyId = companyId, BaseCurrency = "CAD", QuoteCurrency = "USD", Rate = .5m, RateType = CurrencyRateType.Closing, EffectiveOn = new DateOnly(2026, 6, 1), Source = "Test document rate", SourceReference = "https://example.test/cad-usd/2026-06-01" });
            await db.SaveChangesAsync();
        }

        // Real base-currency order/receive pipeline -- only inventory cost flows through this, no bill
        // price. Unit cost (25) is chosen to exactly match the foreign bill's translated per-unit base
        // cost below, so the return's purchase-price-variance amount is zero and the only journal lines
        // in play are inventory and accounts payable.
        var orderResult = await transactions.SavePurchaseOrderAsync(new(null, vendorId, "PO-FXSUPRET-1", new DateOnly(2026, 6, 1), null, "Foreign supplier-return provenance test", [new(itemId, "Foreign compression fittings", 2m, 25m)])); Assert.True(orderResult.Succeeded, orderResult.ErrorMessage);
        ...
        var receiptResult = await transactions.ReceivePurchaseOrderAsync(new(orderResult.Id!.Value, "RCV-FXSUPRET-1", new DateOnly(2026, 6, 1), [new(orderLineId, 2m)], orderToken)); Assert.True(receiptResult.Succeeded, receiptResult.ErrorMessage);
        ...
        // Seed the source (partially paid) foreign bill directly: 2 units at 50 CAD = 100 CAD, rate .5 to
        // base => 50 base. LineTotal/UnitCost are transaction-currency; BaseLineTotal is its base
        // equivalent -- matching how AccountingTransactionService.PurchaseInvoiceMatches.cs populates
        // VendorBillLine elsewhere. BalanceDue/TransactionBalanceDue start partially paid (10/20 of
        // 50/100) so shipping the return both auto-applies part of the credit to this source bill AND
        // leaves the rest available for a separate application below.
        var sourceBillId = Guid.NewGuid(); var sourceBillLineId = Guid.NewGuid();
        await using (var db = await factory.CreateDbContextAsync())
        {
            db.VendorBills.Add(new VendorBill { Id = sourceBillId, CompanyId = companyId, VendorId = vendorId, BillNumber = "BILL-FXSUPRET-1", BillDate = new DateOnly(2026, 6, 1), DueDate = new DateOnly(2026, 7, 1), Status = "Partial", TotalAmount = 50m, BalanceDue = 10m, TransactionCurrency = "CAD", TransactionTotalAmount = 100m, TransactionBalanceDue = 20m, ExchangeRateId = documentRateId, ExchangeRateToBase = .5m, ExchangeRateEffectiveOn = new DateOnly(2026, 6, 1), ExchangeRateSource = "Test document rate", InventoryReceiptId = receiptResult.Id, ConcurrencyToken = Guid.NewGuid().ToString("N") });
        ...
        var authorizationResult = await transactions.AuthorizeSupplierReturnAsync(new(receiptResult.Id!.Value, "SRA-FXSUPRET-1", new DateOnly(2026, 6, 2), "Vendor shipped wrong fittings", [new(receiptLineId, 1m)], receiptToken)); Assert.True(authorizationResult.Succeeded, authorizationResult.ErrorMessage);
        ...
        // Returning 1 of 2 units: proportional (not final) split of both the transaction- and
        // base-currency vendor-credit amounts, with zero purchase-price variance (see the unit-cost note
        // above) and zero GRNI reduction (the whole matched quantity is already invoiced).
        var shipmentResult = await transactions.ShipSupplierReturnAsync(new(authorizationResult.Id!.Value, "SRS-FXSUPRET-1", new DateOnly(2026, 6, 3), null, null, [new(authorizationLineId, 1m)], authorizationToken)); Assert.True(shipmentResult.Succeeded, shipmentResult.ErrorMessage);
        string shipmentToken; await using (var db = await factory.CreateDbContextAsync())
        {
            ...
            Assert.True(shipment.CreatesVendorCredit); Assert.Equal(25m, shipment.VendorCreditAmount); Assert.Equal(50m, shipment.TransactionVendorCreditAmount); Assert.Equal("CAD", shipment.TransactionCurrency); Assert.Equal(.5m, shipment.ExchangeRateToBase); Assert.Equal(documentRateId, shipment.ExchangeRateId);
            Assert.Equal(25m, shipment.TotalAmount); // purchase-price variance is zero by construction
            Assert.Equal(10m, shipment.SourceAppliedAmount); Assert.Equal(20m, shipment.TransactionSourceAppliedAmount); Assert.Equal(10m, shipment.AppliedAmount); Assert.Equal(20m, shipment.TransactionAppliedAmount);
            var sourceBill = await db.VendorBills.SingleAsync(x => x.Id == sourceBillId); Assert.Equal(0m, sourceBill.BalanceDue); Assert.Equal(0m, sourceBill.TransactionBalanceDue);
            Assert.Equal(startingPayables - 25m, await db.Accounts.Where(x => x.OperationalRole == AccountingAccountRoles.AccountsPayable).Select(x => x.CurrentBalance).SingleAsync());
            Assert.Equal(startingVendorBalance - 25m, await db.Vendors.Where(x => x.Id == vendorId).Select(x => x.OpenBalance).SingleAsync());
        }

        // A second, unrelated foreign bill for the same vendor, at the same rate, to apply the remaining credit to.
        var otherBillId = Guid.NewGuid();
        await using (var db = await factory.CreateDbContextAsync())
        {
            db.VendorBills.Add(new VendorBill { Id = otherBillId, CompanyId = companyId, VendorId = vendorId, BillNumber = "BILL-FXSUPRET-OTHER", BillDate = new DateOnly(2026, 6, 1), DueDate = new DateOnly(2026, 7, 1), Status = "Open", TotalAmount = 40m, BalanceDue = 40m, TransactionCurrency = "CAD", TransactionTotalAmount = 80m, TransactionBalanceDue = 80m, ExchangeRateId = documentRateId, ExchangeRateToBase = .5m, ExchangeRateEffectiveOn = new DateOnly(2026, 6, 1), ExchangeRateSource = "Test document rate", ConcurrencyToken = Guid.NewGuid().ToString("N") });
            await db.SaveChangesAsync(); // hand-inserted like the source bill, so it never touched vendor.OpenBalance -- only its own BalanceDue/TransactionBalanceDue matter for the application below.
        }

        // Applying more than the remaining available transaction-currency credit (30 CAD) fails.
        var overApplication = await transactions.ApplySupplierReturnCreditAsync(new(shipmentResult.Id!.Value, otherBillId, new DateOnly(2026, 6, 4), 30.01m, shipmentToken)); Assert.False(overApplication.Succeeded); Assert.Contains("exceeds", overApplication.ErrorMessage, StringComparison.OrdinalIgnoreCase);

        var applicationResult = await transactions.ApplySupplierReturnCreditAsync(new(shipmentResult.Id!.Value, otherBillId, new DateOnly(2026, 6, 4), 30m, shipmentToken)); Assert.True(applicationResult.Succeeded, applicationResult.ErrorMessage);
        string applicationToken; await using (var db = await factory.CreateDbContextAsync())
        {
            var application = await db.SupplierReturnCreditApplications.SingleAsync(x => x.Id == applicationResult.Id); applicationToken = application.ConcurrencyToken;
            Assert.Equal(30m, application.TransactionAmount); Assert.Equal(15m, application.Amount);
            var shipment = await db.SupplierReturnShipments.SingleAsync(x => x.Id == shipmentResult.Id); Assert.Equal(50m, shipment.TransactionAppliedAmount); Assert.Equal(25m, shipment.AppliedAmount);
            var otherBill = await db.VendorBills.SingleAsync(x => x.Id == otherBillId); Assert.Equal(50m, otherBill.TransactionBalanceDue); Assert.Equal(25m, otherBill.BalanceDue);
        }

        // Exact reversal, newest to oldest: the application, then the physical return shipment.
        Assert.True((await transactions.ReverseSupplierReturnCreditApplicationAsync(new(applicationResult.Id!.Value, new DateOnly(2026, 6, 5), "Apply to a different bill instead", applicationToken))).Succeeded);
        await using (var db = await factory.CreateDbContextAsync())
        {
            var shipment = await db.SupplierReturnShipments.SingleAsync(x => x.Id == shipmentResult.Id); Assert.Equal(20m, shipment.TransactionAppliedAmount); Assert.Equal(10m, shipment.AppliedAmount); shipmentToken = shipment.ConcurrencyToken;
            var otherBill = await db.VendorBills.SingleAsync(x => x.Id == otherBillId); Assert.Equal(80m, otherBill.TransactionBalanceDue); Assert.Equal(40m, otherBill.BalanceDue);
        }
        ...
        // The exact bug this proves: TransactionBalanceDue must be restored by the source bill's own
        // TransactionSourceAppliedAmount (20), not by its base-currency SourceAppliedAmount (10) -- the
        // same class of mistake CreditCustomerReturnAsync's reversal path caught and fixed on the
        // customer side.
        var restoredSourceBill = await afterFx.VendorBills.SingleAsync(x => x.Id == sourceBillId);
