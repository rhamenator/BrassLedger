    ...
    IHttpContextAccessor httpContextAccessor) : IAccountingTransactionService
{
    ...
    public async Task<TransactionResult> RecordVendorCreditAsync(RecordVendorCreditRequest request, CancellationToken cancellationToken = default)
    {
        ...
        var rateBasis = (request.RateBasis ?? string.Empty).Trim();
        var isForeign = bill.ExchangeRateId.HasValue;
        if (isForeign)
        {
            if (rateBasis is not ("OriginalDocumentRate" or "AdjustmentDateRate")) return TransactionResult.Failure("A foreign-currency vendor credit must use the OriginalDocumentRate or AdjustmentDateRate rate basis.");
        }
        else if (!string.IsNullOrWhiteSpace(rateBasis)) return TransactionResult.Failure("Do not select a rate basis for a base-currency bill adjustment.");
        if (bill.Status == "Voided" || transactionAmount > bill.TransactionBalanceDue) return TransactionResult.Failure("The credit cannot exceed the open bill balance or target a voided bill.");
        ...
        var carryingReleased = transactionAmount == bill.TransactionBalanceDue ? bill.BalanceDue : RoundCurrency(bill.BalanceDue * transactionAmount / bill.TransactionBalanceDue);
        var translatedAmount = carryingReleased;
        Guid? adjustmentRateId = null; var adjustmentRateToBase = 1m; DateOnly? adjustmentRateEffectiveOn = null; var adjustmentRateSource = string.Empty; var adjustmentRateSourceReference = string.Empty;
        if (isForeign)
        {
            if (rateBasis == "OriginalDocumentRate")
            {
                translatedAmount = RoundCurrency(transactionAmount * bill.ExchangeRateToBase);
                adjustmentRateId = bill.ExchangeRateId; adjustmentRateToBase = bill.ExchangeRateToBase; adjustmentRateEffectiveOn = bill.ExchangeRateEffectiveOn; adjustmentRateSource = bill.ExchangeRateSource; adjustmentRateSourceReference = bill.ExchangeRateSourceReference;
            }
            else
            {
                var (creditRate, creditRateError) = await ResolveTransactionRateAsync(db, companyId, bill.TransactionCurrency, request.ExchangeRateId, request.AdjustmentDate, cancellationToken);
                if (creditRateError is not null) return TransactionResult.Failure(creditRateError);
                translatedAmount = creditRate!.ToBase(transactionAmount);
                adjustmentRateId = creditRate.ExchangeRateId; adjustmentRateToBase = creditRate.FactorToBase; adjustmentRateEffectiveOn = creditRate.EffectiveOn; adjustmentRateSource = creditRate.Source; adjustmentRateSourceReference = creditRate.SourceReference;
            }
        }
        if (translatedAmount <= 0m || carryingReleased <= 0m) return TransactionResult.Failure("The credit must produce positive translated and carrying amounts after conversion.");
        // A vendor credit releases a liability (AP) on the debit side against the offset on the
        // credit side -- the same shape as a refund, not a customer credit memo -- so the gain/loss
        // sign matches the refund convention: carrying released minus the translated other side.
        var realizedGainLoss = isForeign ? carryingReleased - translatedAmount : 0m;

        await using var transaction = await db.Database.BeginTransactionAsync(cancellationToken);
        var adjustmentId = Guid.NewGuid();
        var lines = new List<JournalLineRequest> { new(OperationalRoleReference(AccountingAccountRoles.AccountsPayable), carryingReleased, 0, "Reduce payable"), new(offset.Number, 0, translatedAmount, "Vendor credit") };
        if (realizedGainLoss > 0m) lines.Add(new(OperationalRoleReference(AccountingAccountRoles.ForeignExchangeGain), 0, realizedGainLoss, "Realized foreign-exchange gain"));
        if (realizedGainLoss < 0m) lines.Add(new(OperationalRoleReference(AccountingAccountRoles.ForeignExchangeLoss), -realizedGainLoss, 0, "Realized foreign-exchange loss"));
        var posting = await PostAsync(db, companyId, request.AdjustmentDate, "Accounts Payable", request.Reference, request.Reason,
            ...
            allowControlAccounts: true, sourceDocumentId: adjustmentId, sourceDocumentType: "SubledgerAdjustment", resolveOperationalRoles: true);
        if (!posting.Succeeded) return posting;
        bill.BalanceDue -= carryingReleased;
        bill.TransactionBalanceDue -= transactionAmount;
        ...
        vendor.OpenBalance -= carryingReleased;
        var adjustment = CreateAdjustment(adjustmentId, companyId, "Payables", "VendorCredit", bill.VendorId, request.VendorBillId, null, null, request.AdjustmentDate, translatedAmount, request.Reference, request.Reason, offset.Number, posting.Id!.Value, bill.TransactionCurrency);
        adjustment.TransactionAmount = transactionAmount;
        adjustment.CarryingAmount = carryingReleased;
        adjustment.RateBasis = isForeign ? rateBasis : "BaseCurrency";
        adjustment.ExchangeRateId = adjustmentRateId;
    ...
    public async Task<TransactionResult> RefundUnappliedPaymentAsync(RefundUnappliedPaymentRequest request, CancellationToken cancellationToken = default)
    {
        ...
        var cashAmount = refundRate!.ToBase(transactionAmount);
        var carryingAmount = transactionAmount == payment.TransactionUnappliedAmount
            ? payment.UnappliedAmount
            : RoundCurrency(payment.UnappliedAmount * transactionAmount / payment.TransactionUnappliedAmount);
        if (cashAmount <= 0m || carryingAmount <= 0m) return TransactionResult.Failure("The refund must produce positive cash and carrying amounts after conversion.");
        ...
        if (payment.Direction == "CustomerReceipt" && bank.CurrentBalance < cashAmount) return TransactionResult.Failure("The bank account does not have sufficient book balance for this refund.");
        ...
        var realizedGainLoss = customerRefund ? carryingAmount - cashAmount : cashAmount - carryingAmount;
        ...
        var lines = customerRefund
            ? new List<JournalLineRequest> { new(offsetAccount, carryingAmount, 0, "Release customer deposit"), new(cashAccount, 0, cashAmount, "Customer refund") }
            : [new(cashAccount, cashAmount, 0, "Vendor refund received"), new(offsetAccount, 0, carryingAmount, "Release vendor advance")];
        if (realizedGainLoss > 0m) lines.Add(new(OperationalRoleReference(AccountingAccountRoles.ForeignExchangeGain), 0, realizedGainLoss, "Realized foreign-exchange gain"));
        if (realizedGainLoss < 0m) lines.Add(new(OperationalRoleReference(AccountingAccountRoles.ForeignExchangeLoss), -realizedGainLoss, 0, "Realized foreign-exchange loss"));
        var posting = await PostAsync(db, companyId, request.RefundDate, customerRefund ? "Accounts Receivable" : "Accounts Payable", request.Reference, request.Reason, lines, cancellationToken, bank.Id, true, adjustmentId, "SubledgerAdjustment", resolveOperationalRoles: true);
        if (!posting.Succeeded) return posting;
        payment.UnappliedAmount -= carryingAmount;
        payment.TransactionUnappliedAmount -= transactionAmount;
        payment.ConcurrencyToken = Guid.NewGuid().ToString("N");
        bank.CurrentBalance += customerRefund ? -cashAmount : cashAmount;
        bank.UnreconciledAmount += cashAmount;
        bank.ConcurrencyToken = Guid.NewGuid().ToString("N");
        var adjustment = CreateAdjustment(adjustmentId, companyId, subledger, customerRefund ? "CustomerDepositRefund" : "VendorAdvanceRefund", payment.CounterpartyId, null, payment.Id, bank.Id, request.RefundDate, cashAmount, request.Reference, request.Reason, offsetAccount, posting.Id!.Value, payment.TransactionCurrency);
        adjustment.TransactionCurrency = refundRate.TransactionCurrency;
        adjustment.TransactionAmount = transactionAmount;
        adjustment.CarryingAmount = carryingAmount;
        adjustment.RateBasis = refundRate.IsForeign ? "AdjustmentDateRate" : "BaseCurrency";
        adjustment.ExchangeRateId = refundRate.ExchangeRateId;
        adjustment.ExchangeRateToBase = refundRate.FactorToBase;
        adjustment.ExchangeRateEffectiveOn = refundRate.EffectiveOn;
        adjustment.ExchangeRateSource = refundRate.Source;
        adjustment.ExchangeRateSourceReference = refundRate.SourceReference;
        adjustment.RealizedGainLoss = realizedGainLoss;
        db.SubledgerAdjustments.Add(adjustment);
        AddAdjustmentAudit(db, adjustment, "subledger-adjustment.posted");
    ...
    public async Task<TransactionResult> ReverseSubledgerAdjustmentAsync(ReverseSubledgerAdjustmentRequest request, CancellationToken cancellationToken = default)
    {
        ...
        if (adjustment.PaymentId.HasValue)
        {
            var siblingRefunds = await db.SubledgerAdjustments.Where(item => item.CompanyId == companyId && item.PaymentId == adjustment.PaymentId && item.Status == "Posted" && item.Id != adjustment.Id).Select(item => new { item.AdjustmentDate, item.CreatedAtUtc }).ToListAsync(cancellationToken);
            if (siblingRefunds.Any(item => item.AdjustmentDate > adjustment.AdjustmentDate || (item.AdjustmentDate == adjustment.AdjustmentDate && item.CreatedAtUtc > adjustment.CreatedAtUtc)))
                return TransactionResult.Failure("Reverse later refunds for this payment before reversing this adjustment.");
        }
        ...
        else if (adjustment.Kind == "VendorCredit")
        {
            ...
            bill.BalanceDue += adjustment.CarryingAmount; bill.TransactionBalanceDue += adjustment.TransactionAmount; bill.Status = bill.BalanceDue == bill.TotalAmount ? "Open" : "Partial"; bill.ConcurrencyToken = Guid.NewGuid().ToString("N");
            (await db.Vendors.SingleAsync(item => item.Id == adjustment.CounterpartyId && item.CompanyId == companyId, cancellationToken)).OpenBalance += adjustment.CarryingAmount;
        }
        else if (adjustment.Kind is "CustomerDepositRefund" or "VendorAdvanceRefund")
        {
            ...
            payment.UnappliedAmount += adjustment.CarryingAmount; payment.TransactionUnappliedAmount += adjustment.TransactionAmount; payment.ConcurrencyToken = Guid.NewGuid().ToString("N");
        ...
        adjustment.Status = "Reversed"; adjustment.ReversalJournalEntryId = reversal.Id; adjustment.ReversedByUserId = ResolveUserId(); adjustment.ReversedAtUtc = DateTimeOffset.UtcNow; adjustment.ReversalDate = request.ReversalDate; adjustment.ReversalReason = request.Reason.Trim(); adjustment.ConcurrencyToken = Guid.NewGuid().ToString("N");
    ...
    private async Task<TransactionResult> VoidSubledgerDocumentAsync(VoidSubledgerDocumentRequest request, bool receivable, CancellationToken cancellationToken)
    {
        ...
        var companyId = await ResolveCompanyIdAsync(db, cancellationToken);
        await using var transaction = await db.Database.BeginTransactionAsync(cancellationToken);
        var reference = string.Empty; var documentDate = default(DateOnly); var amount = 0m; var transactionAmount = 0m; var transactionCurrency = string.Empty; var counterpartyId = Guid.Empty;
        Guid? exchangeRateId = null; var exchangeRateToBase = 1m; DateOnly? exchangeRateEffectiveOn = null; var exchangeRateSource = string.Empty; var exchangeRateSourceReference = string.Empty;
        ...
        if (receivable)
        {
            ...
            reference = invoice.InvoiceNumber; documentDate = invoice.InvoiceDate; amount = invoice.TotalAmount; transactionAmount = invoice.TransactionTotalAmount; transactionCurrency = invoice.TransactionCurrency; counterpartyId = invoice.CustomerId;
            exchangeRateId = invoice.ExchangeRateId; exchangeRateToBase = invoice.ExchangeRateToBase; exchangeRateEffectiveOn = invoice.ExchangeRateEffectiveOn; exchangeRateSource = invoice.ExchangeRateSource; exchangeRateSourceReference = invoice.ExchangeRateSourceReference;
        }
        else
        {
            ...
            reference = bill.BillNumber; documentDate = bill.BillDate; amount = bill.TotalAmount; transactionAmount = bill.TransactionTotalAmount; transactionCurrency = bill.TransactionCurrency; counterpartyId = bill.VendorId;
            exchangeRateId = bill.ExchangeRateId; exchangeRateToBase = bill.ExchangeRateToBase; exchangeRateEffectiveOn = bill.ExchangeRateEffectiveOn; exchangeRateSource = bill.ExchangeRateSource; exchangeRateSourceReference = bill.ExchangeRateSourceReference;
        }
        ...
        var adjustment = CreateAdjustment(adjustmentId, companyId, receivable ? "Receivables" : "Payables", receivable ? "InvoiceVoid" : "VendorBillVoid", counterpartyId, request.DocumentId, null, null, request.VoidDate, amount, $"VOID-{reference}", request.Reason, string.Empty, reversal.Id!.Value, transactionCurrency);
        adjustment.TransactionAmount = transactionAmount;
        adjustment.RateBasis = exchangeRateId.HasValue ? "OriginalDocumentRate" : "BaseCurrency";
        adjustment.ExchangeRateId = exchangeRateId;
        adjustment.ExchangeRateToBase = exchangeRateToBase;
        adjustment.ExchangeRateEffectiveOn = exchangeRateEffectiveOn;
        adjustment.ExchangeRateSource = exchangeRateSource;
        adjustment.ExchangeRateSourceReference = exchangeRateSourceReference;
    ...
    public async Task<TransactionResult> CreateReconciliationAdjustmentAsync(CreateReconciliationAdjustmentRequest request, CancellationToken cancellationToken = default)
    {
        ...
        bank.ConcurrencyToken = Guid.NewGuid().ToString("N");
        var baseCurrency = await db.Companies.Where(company => company.Id == companyId).Select(company => company.BaseCurrency).SingleAsync(cancellationToken);
        var adjustment = CreateAdjustment(adjustmentId, companyId, "Banking", "BankReconciliationAdjustment", bank.Id, null, null, bank.Id, request.AdjustmentDate, RoundCurrency(request.Amount), request.Reference, request.Description, offset.Number, posting.Id!.Value, baseCurrency);
        db.SubledgerAdjustments.Add(adjustment);
    ...
    public async Task<PostEmployeePayrollRunRequest?> GetEmployeePayrollRunDraftAsync(Guid payrollRunId, CancellationToken cancellationToken = default)
    {
        ...
        var employees = employeeLines.Select(line =>
        {
            var explicitEarnings = earnings.Where(earning => earning.PayrollRunEmployeeLineId == line.Id && earning.PayrollTimeEntryId is null)
                .Select(earning => new PayrollEarningInput(earning.EarningCode, earning.EarningType, earning.Hours, earning.Rate, earning.Amount, earning.IsTaxable, earning.WorkedOn, earning.WorkState, earning.WorkCounty, earning.WorkCity, earning.WorkSchoolDistrict, W2Reporting: ParseW2Reporting(earning.W2ReportingJson), ProjectJobId: earning.ProjectJobId)).ToArray();
    ...
    public async Task<TransactionResult> SaveEmployeePayrollRunDraftAsync(PostEmployeePayrollRunRequest request, CancellationToken cancellationToken = default)
    {
        ...
        var projectIds = request.Employees.SelectMany(employee => employee.Earnings ?? []).Where(earning => earning.ProjectJobId.HasValue).Select(earning => earning.ProjectJobId!.Value).Distinct().ToArray();
        if (projectIds.Length > 0 && await db.ProjectJobs.CountAsync(project => project.CompanyId == companyId && project.Status == "Active" && projectIds.Contains(project.Id), cancellationToken) != projectIds.Length)
            return TransactionResult.Failure("Every payroll earning project must be active and belong to this company.");
    ...
    public async Task<TransactionResult> PostApprovedPayrollRunAsync(PostApprovedPayrollRunRequest request, CancellationToken cancellationToken = default)
    {
        ...
        var earningLines = await db.PayrollEarningLines.Where(line => employeeLineIds.Contains(line.PayrollRunEmployeeLineId)).ToListAsync(cancellationToken);
        ...
        foreach (var employeeLine in employeeLines)
        {
            ...
            var earningGroups = employeeEarnings.GroupBy(line => new { line.ProjectJobId, line.ProjectPhaseId, line.ProjectCostCodeId, line.DepartmentId, line.ClassId })
                ...
                .ToArray();
            var employerBurden = RoundCurrency(employeeLine.EmployerPayrollTaxes + employeeLine.EmployerBenefitContributions);
            var unallocatedBurden = employerBurden;
            for (var index = 0; index < earningGroups.Length; index++)
            {
                var group = earningGroups[index];
                var allocatedBurden = index == earningGroups.Length - 1
                    ? unallocatedBurden
    ...
    private async Task<TransactionResult> PostAsync(BrassLedgerDbContext db, Guid companyId, DateOnly date, string module, string reference, string description, IReadOnlyList<JournalLineRequest> lines, CancellationToken ct, Guid? bankAccountId = null, bool allowControlAccounts = false, Guid? sourceDocumentId = null, string? sourceDocumentType = null, bool resolveOperationalRoles = false, bool allowClosedProjects = false)
    {
    ...
    private static SubledgerAdjustment CreateAdjustment(Guid id, Guid companyId, string subledger, string kind, Guid counterpartyId, Guid? documentId, Guid? paymentId, Guid? bankAccountId, DateOnly date, decimal amount, string reference, string reason, string offsetAccountNumber, Guid journalEntryId, string transactionCurrency) => new()
    {
        Id = id,
        CompanyId = companyId,
        Subledger = subledger,
        ...
        ExchangeRateEffectiveOn = date,
        ExchangeRateSource = "Base-currency adjustment",
        Reference = reference.Trim(),
        Reason = reason.Trim(),
    ...
    private void AddAdjustmentAudit(BrassLedgerDbContext db, SubledgerAdjustment adjustment, string action)
    {
        ...
        db.BusinessAuditEntries.Add(new BusinessAuditEntry
        {
            ...
            DetailJson = System.Text.Json.JsonSerializer.Serialize(new { adjustment.Subledger, adjustment.Kind, adjustment.CounterpartyId, adjustment.DocumentId, adjustment.PaymentId, adjustment.BankAccountId, adjustment.AdjustmentDate, adjustment.Amount, adjustment.TransactionCurrency, adjustment.TransactionAmount, adjustment.CarryingAmount, adjustment.RateBasis, adjustment.ExchangeRateId, adjustment.ExchangeRateToBase, adjustment.ExchangeRateEffectiveOn, adjustment.ExchangeRateSource, adjustment.ExchangeRateSourceReference, adjustment.RealizedGainLoss, adjustment.Reference, adjustment.Reason, adjustment.OffsetAccountNumber, adjustment.Status, adjustment.JournalEntryId, adjustment.ReversalJournalEntryId, adjustment.ReversalDate }),
            OccurredAtUtc = DateTimeOffset.UtcNow
        });
    }
