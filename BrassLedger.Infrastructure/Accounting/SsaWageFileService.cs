...
{
    private const string FileKind = "EFW2C";
    public const string SupportedLayoutCode = "EFW2C-2026-1024-RCA-RCE-RCW-RCO-RCT-RCU-RCF";
    public const string Supported2025LayoutCode = "EFW2C-1024-RCA-RCE-RCW-RCT-RCF";
    ...
    public async Task<SsaWageFileWorkspace> GetAsync(CancellationToken cancellationToken = default)
    {
        ...
        var configurations = await db.PayrollSsaWageFileConfigurations.AsNoTracking().Where(item => item.CompanyId == companyId && item.FileKind == FileKind).OrderByDescending(item => item.SpecificationTaxYear).ToListAsync(cancellationToken);
    ...
    public async Task<TransactionResult> SaveConfigurationAsync(SaveSsaWageFileConfigurationRequest request, CancellationToken cancellationToken = default)
    {
        ...
        var expectedLayoutCode = LayoutCodeFor(request.SpecificationTaxYear);
        if (request.IsApproved && (request.LayoutCompatibilityCode != expectedLayoutCode || digest.Length != 64 || digest.Any(character => !Uri.IsHexDigit(character)) || notes.Length < 30)) return TransactionResult.Failure($"Approval requires the supported reviewed layout code {expectedLayoutCode}, the official PDF SHA-256, and substantive review notes.");
        ...
        else { if (await db.PayrollSsaWageFileConfigurations.AnyAsync(item => item.CompanyId == companyId && item.SpecificationTaxYear == request.SpecificationTaxYear && item.FileKind == FileKind, cancellationToken)) return TransactionResult.Failure("This company already has an SSA EFW2C configuration for that tax year."); entity = new() { Id = Guid.NewGuid(), CompanyId = companyId, FileKind = FileKind }; db.Add(entity); }
    ...
    public async Task<TransactionResult> GenerateAsync(GenerateSsaWageFileRequest request, CancellationToken cancellationToken = default)
    {
        ...
        var configuration = await db.PayrollSsaWageFileConfigurations.AsNoTracking().SingleOrDefaultAsync(item => item.CompanyId == companyId && item.SpecificationTaxYear == correction.TaxYear && item.FileKind == FileKind && item.IsApproved && item.IsActive, cancellationToken);
        ...
        if (configuration.LayoutCompatibilityCode != LayoutCodeFor(correction.TaxYear)) return TransactionResult.Failure("The active SSA EFW2C configuration was approved for an older encoder layout. Re-review and approve the current layout before generation.");
    ...
    private static string LayoutCodeFor(int year) => year == 2025 ? Supported2025LayoutCode : SupportedLayoutCode;
