...
public sealed partial class AccountingTransactionService
{
        ...
        else if (project.BillingMethod == "TimeAndMaterials")
        {
            ...
            if (request.IncludeLabor)
            {
                ...
                foreach (var entry in candidates.Where(x => !blocked.Contains($"TIME:{x.Id:N}")))
                {
                    ...
                    if (gross > 0m) lines.Add(new("ApprovedTime", entry.Id, $"TIME:{entry.Id:N}", $"{entry.WorkDate:yyyy-MM-dd} {entry.EarningCode} — {entry.Hours:0.####} hours", entry.Hours, rate.HourlyRate, entry.Amount, gross - entry.Amount, gross, 0m, gross, revenueAccount, entry.ProjectPhaseId, entry.ProjectCostCodeId, entry.DepartmentId, entry.ClassId));
                }
            }
    ...
    private static async Task AddEligibleCostLinesAsync(BrassLedgerDbContext db, Guid companyId, Guid projectId, ProjectBillingPreviewRequest request, HashSet<string> blocked, string revenueAccount, bool includePayroll, List<ProjectBillingPreviewLine> lines, CancellationToken cancellationToken)
    {
        ...
        foreach (var item in costs.Where(x => !blocked.Contains($"COST:{x.Line.Id:N}")))
        {
            ...
            lines.Add(new("PostedCost", item.Line.Id, $"COST:{item.Line.Id:N}", $"{item.Entry.PostedOn:yyyy-MM-dd} {item.Entry.Reference} — {item.Line.Description}", 1m, gross, item.Cost, markup, gross, 0m, gross, revenueAccount, item.Line.ProjectPhaseId, item.Line.ProjectCostCodeId, item.Line.DepartmentId, item.Line.ClassId));
        }
    }
        ...
        foreach (var line in sourcedLines)
        {
            ...
            if (line.SourceType == "ApprovedTime")
            {
                var source = await (from entry in db.PayrollTimeEntries
                                    ...
                                    select new { entry.Hours, entry.Amount, entry.WorkDate, entry.ProjectPhaseId, entry.ProjectCostCodeId, entry.DepartmentId, entry.ClassId }).SingleOrDefaultAsync(cancellationToken);
                if (source is null || source.WorkDate > proposal.BillingThrough || source.Hours != line.Quantity || source.Amount != line.SourceCost || source.ProjectPhaseId != line.ProjectPhaseId || source.ProjectCostCodeId != line.ProjectCostCodeId || source.DepartmentId != line.DepartmentId || source.ClassId != line.ClassId)
            ...
            else if (line.SourceType == "PostedCost")
            {
                var source = await (from journalLine in db.JournalEntryLines
                                    ...
                                    select new { Cost = journalLine.Debit - journalLine.Credit, journal.PostedOn, journalLine.ProjectPhaseId, journalLine.ProjectCostCodeId, journalLine.DepartmentId, journalLine.ClassId }).SingleOrDefaultAsync(cancellationToken);
                if (source is null || source.PostedOn > proposal.BillingThrough || source.Cost <= 0m || source.Cost != line.SourceCost || source.ProjectPhaseId != line.ProjectPhaseId || source.ProjectCostCodeId != line.ProjectCostCodeId || source.DepartmentId != line.DepartmentId || source.ClassId != line.ClassId)
