    ...
    public async Task<BusinessWorkspaceSnapshot> GetWorkspaceAsync(CancellationToken cancellationToken = default)
    {
        ...
        return new BusinessWorkspaceSnapshot(
            ...
            Operations: new OperationsWorkspace(
                ...
                InventoryReceipts: inventoryReceipts.Select(receipt => new InventoryReceiptSnapshot(receipt.Id, receipt.PurchaseOrderId, purchaseOrderById.GetValueOrDefault(receipt.PurchaseOrderId)?.OrderNumber ?? "Unavailable", receipt.ReceiptNumber, receipt.ReceivedOn, receipt.Status, receipt.TotalAmount, vendorBillByReceiptId.GetValueOrDefault(receipt.Id)?.Id, receipt.ConcurrencyToken, inventoryReceiptLineLookup[receipt.Id].Select(line => new InventoryReceiptLineSnapshot(line.Id, line.PurchaseOrderLineId, line.InventoryItemId, inventoryItemById.GetValueOrDefault(line.InventoryItemId)?.Sku ?? "Unavailable", line.Sequence, line.Quantity, line.UnitCost, line.LineTotal)).ToArray(), receipt.JournalEntryId, receipt.ReversalJournalEntryId, receipt.WarehouseId, receipt.BinId, InventoryLocationLabel(receipt.WarehouseId, receipt.BinId))).ToArray(),
                InventoryShipments: inventoryShipments.Select(shipment => new InventoryShipmentSnapshot(shipment.Id, shipment.SalesOrderId, salesOrderById.GetValueOrDefault(shipment.SalesOrderId)?.OrderNumber ?? "Unavailable", shipment.ShipmentNumber, shipment.ShippedOn, shipment.Status, shipment.TotalCost, shipment.SalesInvoiceId, shipment.ConcurrencyToken, inventoryShipmentLineLookup[shipment.Id].Select(line => new InventoryShipmentLineSnapshot(line.Id, line.SalesOrderLineId, line.InventoryItemId, inventoryItemById.GetValueOrDefault(line.InventoryItemId)?.Sku ?? "Unavailable", line.Sequence, line.Quantity, line.UnitCost, line.TotalCost)).ToArray(), shipment.JournalEntryId, shipment.ReversalJournalEntryId, shipment.WarehouseId, shipment.BinId, InventoryLocationLabel(shipment.WarehouseId, shipment.BinId))).ToArray(),
