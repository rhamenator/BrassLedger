    ...
    IHttpContextAccessor httpContextAccessor) : IBusinessWorkspaceService
{
    public async Task<BusinessWorkspaceSnapshot> GetWorkspaceAsync(CancellationToken cancellationToken = default)
    {
        ...
        var trackingDimensions = await dbContext.TrackingDimensionValues.AsNoTracking().Where(x => x.CompanyId == company.Id).OrderBy(x => x.DimensionType).ThenBy(x => x.Code).ToListAsync(cancellationToken);
        ...
        var projectPhases = await dbContext.ProjectPhases.AsNoTracking().Where(x => x.CompanyId == company.Id).OrderBy(x => x.ProjectJobId).ThenBy(x => x.Code).ToListAsync(cancellationToken);
        var projectCostCodes = await dbContext.ProjectCostCodes.AsNoTracking().Where(x => x.CompanyId == company.Id).OrderBy(x => x.Code).ToListAsync(cancellationToken);
        var projectBudgetAllocations = await dbContext.ProjectBudgetAllocations.AsNoTracking().Where(x => x.CompanyId == company.Id).OrderBy(x => x.ProjectJobId).ThenBy(x => x.PeriodStart).ToListAsync(cancellationToken);
        ...
        var trackingDimensionById = trackingDimensions.ToDictionary(value => value.Id);
        ...
        var projectPhaseById = projectPhases.ToDictionary(phase => phase.Id);
        var projectCostCodeById = projectCostCodes.ToDictionary(code => code.Id);
        var postedRetainageReleases = projectBillingProposals.Where(proposal => proposal.Status == "Posted" && proposal.RetainageReleaseOfProposalId.HasValue).ToLookup(proposal => proposal.RetainageReleaseOfProposalId!.Value);
        var retainageAsOf = DateOnly.FromDateTime(DateTime.Today);
        var projectRetainageAging = projectBillingProposals.Where(proposal => proposal.Status == "Posted" && proposal.BillingBasis != "RetainageRelease" && proposal.RetainageAmount > 0m).Select(proposal =>
        {
            var released = postedRetainageReleases[proposal.Id].Sum(release => release.InvoiceAmount);
            var outstanding = proposal.RetainageAmount - released;
            var ageDays = Math.Max(0, retainageAsOf.DayNumber - proposal.InvoiceDate.DayNumber);
            return new ProjectRetainageAgingSnapshot(proposal.Id, proposal.ProjectJobId, projectById.GetValueOrDefault(proposal.ProjectJobId)?.JobNumber ?? "Unavailable", customerNames.GetValueOrDefault(proposal.CustomerId, "Unknown customer"), proposal.InvoiceNumber, proposal.InvoiceDate, ageDays, proposal.RetainageAmount, released, outstanding, ageDays <= 30 ? outstanding : 0m, ageDays is >= 31 and <= 60 ? outstanding : 0m, ageDays is >= 61 and <= 90 ? outstanding : 0m, ageDays > 90 ? outstanding : 0m);
        ...
        return new BusinessWorkspaceSnapshot(
            ...
            GeneralLedger: new GeneralLedgerWorkspace(
                ...
                RecentEntries: journalEntries.Select(x => new JournalEntrySnapshot(x.EntryNumber, x.PostedOn, x.SourceModule, x.Description, x.TotalAmount, x.Id, x.Reference, x.Status, x.ReversalOfJournalEntryId, x.ReversedByJournalEntryId, x.RejectedAtUtc, x.DecisionReason, x.ConcurrencyToken, journalEntryLines.Where(line => line.JournalEntryId == x.Id).Select(line => new JournalEntryLineSnapshot(accounts.Single(account => account.Id == line.AccountId).Number, line.Description, line.Debit, line.Credit, line.ProjectJobId, line.ProjectJobId.HasValue && projectById.TryGetValue(line.ProjectJobId.Value, out var project) ? project.JobNumber : string.Empty, line.ProjectPhaseId, line.ProjectPhaseId.HasValue && projectPhaseById.TryGetValue(line.ProjectPhaseId.Value, out var phase) ? phase.Code : string.Empty, line.ProjectCostCodeId, line.ProjectCostCodeId.HasValue && projectCostCodeById.TryGetValue(line.ProjectCostCodeId.Value, out var costCode) ? costCode.Code : string.Empty, line.DepartmentId, line.DepartmentId.HasValue && trackingDimensionById.TryGetValue(line.DepartmentId.Value, out var department) ? department.Code : string.Empty, line.ClassId, line.ClassId.HasValue && trackingDimensionById.TryGetValue(line.ClassId.Value, out var accountingClass) ? accountingClass.Code : string.Empty)).ToArray())).ToArray(),
                TrackingDimensions: trackingDimensions.Select(value => new TrackingDimensionValueSnapshot(value.Id, value.DimensionType, value.ParentTrackingDimensionValueId, value.Code, value.Name, value.Description, value.EffectiveFrom, value.EffectiveThrough, value.IsActive, value.ConcurrencyToken)).ToArray()),
            Receivables: new ReceivablesWorkspace(
                ...
                Invoices: invoices.Select(x => new InvoiceSnapshot(
                    ...
                    x.BalanceDue,
                    x.Id,
                    invoiceLineLookup[x.Id].Select(line => new InvoiceLineSnapshot(line.Sequence, line.Description, line.Quantity, line.UnitPrice, line.DiscountAmount, line.TaxAmount, line.LineTotal, accountNumbersById.GetValueOrDefault(line.RevenueAccountId, "Unavailable"), line.ProjectJobId, line.ProjectJobId.HasValue ? projectById.GetValueOrDefault(line.ProjectJobId.Value)?.JobNumber ?? "Unavailable" : string.Empty, line.ProjectPhaseId, line.ProjectPhaseId.HasValue ? projectPhaseById.GetValueOrDefault(line.ProjectPhaseId.Value)?.Code ?? "Unavailable" : string.Empty, line.ProjectCostCodeId, line.ProjectCostCodeId.HasValue ? projectCostCodeById.GetValueOrDefault(line.ProjectCostCodeId.Value)?.Code ?? "Unavailable" : string.Empty, line.DepartmentId, line.DepartmentId.HasValue ? trackingDimensionById.GetValueOrDefault(line.DepartmentId.Value)?.Code ?? "Unavailable" : string.Empty, line.ClassId, line.ClassId.HasValue ? trackingDimensionById.GetValueOrDefault(line.ClassId.Value)?.Code ?? "Unavailable" : string.Empty)).ToArray(),
            ...
            Payables: new PayablesWorkspace(
                ...
                Bills: vendorBills.Select(x => new BillSnapshot(
                    ...
                    x.BalanceDue,
                    x.Id,
                    vendorBillLineLookup[x.Id].Select(line => new BillLineSnapshot(line.Sequence, line.Description, line.Quantity, line.UnitCost, line.DiscountAmount, line.TaxAmount, line.LineTotal, accountNumbersById.GetValueOrDefault(line.ExpenseAccountId, "Unavailable"), line.ProjectJobId, line.ProjectJobId.HasValue ? projectById.GetValueOrDefault(line.ProjectJobId.Value)?.JobNumber ?? "Unavailable" : string.Empty, line.ProjectPhaseId, line.ProjectPhaseId.HasValue ? projectPhaseById.GetValueOrDefault(line.ProjectPhaseId.Value)?.Code ?? "Unavailable" : string.Empty, line.ProjectCostCodeId, line.ProjectCostCodeId.HasValue ? projectCostCodeById.GetValueOrDefault(line.ProjectCostCodeId.Value)?.Code ?? "Unavailable" : string.Empty, line.DepartmentId, line.DepartmentId.HasValue ? trackingDimensionById.GetValueOrDefault(line.DepartmentId.Value)?.Code ?? "Unavailable" : string.Empty, line.ClassId, line.ClassId.HasValue ? trackingDimensionById.GetValueOrDefault(line.ClassId.Value)?.Code ?? "Unavailable" : string.Empty)).ToArray(),
            ...
            Operations: new OperationsWorkspace(
                ...
                PurchaseOrders: purchaseOrders.Select(x => new PurchaseOrderSnapshot(
                    ...
                    x.Notes,
                    x.ConcurrencyToken,
                    purchaseOrderLineLookup[x.Id].Select(line => new PurchaseOrderLineSnapshot(line.Id, line.Sequence, line.InventoryItemId, inventoryItemById.GetValueOrDefault(line.InventoryItemId)?.Sku ?? "Unavailable", line.Description, line.OrderedQuantity, line.UnitCost, line.ReceivedQuantity, line.InvoicedQuantity, line.LineTotal, line.ReturnedQuantity, line.CreditedQuantity, line.ProjectJobId, line.ProjectPhaseId, line.ProjectCostCodeId, line.DepartmentId, line.ClassId)).ToArray())).ToArray(),
                ...
                SupplierReturnShipments: supplierReturnShipments.Select(item =>
                {
                    ...
                    return new SupplierReturnShipmentSnapshot(
                        ...
                        item.TransactionCurrency, item.TransactionVendorCreditAmount - item.TransactionAppliedAmount - item.TransactionRefundedAmount,
            ...
            Projects: new ProjectsWorkspace(
                ...
                LedgerLines: projectLedgerRows.Select(row => new ProjectLedgerLineSnapshot(row.LineId, row.ProjectJobId, row.PostedOn, row.Reference, row.SourceModule, row.AccountNumber, row.AccountName, row.LineDescription, row.Debit, row.Credit, row.AccountType == AccountType.Expense ? row.Debit - row.Credit : 0m, row.AccountType == AccountType.Revenue ? row.Credit - row.Debit : 0m, row.EntryId, row.ProjectPhaseId, row.ProjectPhaseId.HasValue && projectPhaseById.TryGetValue(row.ProjectPhaseId.Value, out var phase) ? phase.Code : string.Empty, row.ProjectCostCodeId, row.ProjectCostCodeId.HasValue && projectCostCodeById.TryGetValue(row.ProjectCostCodeId.Value, out var costCode) ? costCode.Code : string.Empty)).ToArray(),
                ...
                BillingRates: projectBillingRates.Select(rate => new ProjectBillingRateSnapshot(rate.Id, rate.ProjectJobId, projectById.GetValueOrDefault(rate.ProjectJobId)?.JobNumber ?? "Unavailable", rate.EarningCode, rate.HourlyRate, rate.EffectiveOn, rate.EffectiveThrough, rate.IsActive, rate.ConcurrencyToken)).ToArray(),
                BillingProposals: projectBillingProposals.Select(proposal => new ProjectBillingProposalSnapshot(proposal.Id, proposal.ProjectJobId, projectById.GetValueOrDefault(proposal.ProjectJobId)?.JobNumber ?? "Unavailable", proposal.SubledgerDocumentWorkflowId, proposal.RetainageReleaseOfProposalId, proposal.InvoiceNumber, proposal.BillingThrough, proposal.InvoiceDate, proposal.DueDate, proposal.BillingBasis, proposal.ProgressPercentToDate, proposal.CostMarkupPercent, proposal.ContractAmountSnapshot, proposal.RetainagePercentSnapshot, proposal.GrossAmount, proposal.RetainageAmount, proposal.InvoiceAmount, proposal.RevenueAccountNumber, proposal.Description, proposal.Status, proposal.PreparedAtUtc, proposal.CancelledAtUtc, proposal.CancellationReason, proposal.ConcurrencyToken, projectBillingLineLookup[proposal.Id].Select(line => new ProjectBillingLineSnapshot(line.Sequence, line.SourceType, line.SourceId, line.SourceKey, line.Description, line.Quantity, line.UnitPrice, line.SourceCost, line.MarkupAmount, line.GrossAmount, line.RetainageAmount, line.InvoiceAmount, line.RevenueAccountNumber, line.ProjectPhaseId, line.ProjectPhaseId.HasValue && projectPhaseById.TryGetValue(line.ProjectPhaseId.Value, out var phase) ? phase.Code : string.Empty, line.ProjectCostCodeId, line.ProjectCostCodeId.HasValue && projectCostCodeById.TryGetValue(line.ProjectCostCodeId.Value, out var costCode) ? costCode.Code : string.Empty, line.DepartmentId, line.DepartmentId.HasValue && trackingDimensionById.TryGetValue(line.DepartmentId.Value, out var department) ? department.Code : string.Empty, line.ClassId, line.ClassId.HasValue && trackingDimensionById.TryGetValue(line.ClassId.Value, out var accountingClass) ? accountingClass.Code : string.Empty)).ToArray())).ToArray(),
                ...
                Phases: projectPhases.Select(phase => new ProjectPhaseSnapshot(phase.Id, phase.ProjectJobId, projectById.GetValueOrDefault(phase.ProjectJobId)?.JobNumber ?? "Unavailable", phase.ParentProjectPhaseId, phase.Code, phase.Name, phase.Kind, phase.Description, phase.StartsOn, phase.EndsOn, phase.IsActive, phase.ConcurrencyToken)).ToArray(),
                CostCodes: projectCostCodes.Select(code => new ProjectCostCodeSnapshot(code.Id, code.Code, code.Name, code.Category, code.Description, code.IsActive, code.ConcurrencyToken)).ToArray(),
                BudgetAllocations: projectBudgetAllocations.Select(allocation => new ProjectBudgetAllocationSnapshot(allocation.Id, allocation.ProjectJobId, projectById.GetValueOrDefault(allocation.ProjectJobId)?.JobNumber ?? "Unavailable", allocation.ProjectPhaseId, projectPhases.SingleOrDefault(phase => phase.Id == allocation.ProjectPhaseId)?.Code ?? string.Empty, allocation.ProjectCostCodeId, projectCostCodes.SingleOrDefault(code => code.Id == allocation.ProjectCostCodeId)?.Code ?? string.Empty, allocation.AccountId, accounts.SingleOrDefault(account => account.Id == allocation.AccountId)?.Number ?? string.Empty, allocation.PeriodStart, allocation.PeriodEnd, allocation.BudgetAmount, allocation.ForecastAmount, allocation.Notes, allocation.ConcurrencyToken)).ToArray()),
