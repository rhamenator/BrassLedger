    ...
    IHttpContextAccessor httpContextAccessor) : IBusinessWorkspaceService
{
    public async Task<BusinessWorkspaceSnapshot> GetWorkspaceAsync(CancellationToken cancellationToken = default)
    {
        ...
        var vendorBillByReceiptId = vendorBills.Where(bill => bill.InventoryReceiptId.HasValue).ToDictionary(bill => bill.InventoryReceiptId!.Value);
        ...
        return new BusinessWorkspaceSnapshot(
            ...
            Operations: new OperationsWorkspace(
                ...
                SupplierReturnAuthorizations: supplierReturnAuthorizations.Select(item => new SupplierReturnAuthorizationSnapshot(item.Id, item.InventoryReceiptId, inventoryReceiptById.GetValueOrDefault(item.InventoryReceiptId)?.ReceiptNumber ?? "Unavailable", item.PurchaseOrderId, purchaseOrderById.GetValueOrDefault(item.PurchaseOrderId)?.OrderNumber ?? "Unavailable", item.VendorId, vendorNames.GetValueOrDefault(item.VendorId, "Unknown vendor"), item.ReturnNumber, item.AuthorizedOn, item.Reason, item.Status, item.CancellationReason, item.ConcurrencyToken, supplierReturnAuthorizationLineLookup[item.Id].Select(line => new SupplierReturnAuthorizationLineSnapshot(line.Id, line.InventoryReceiptLineId, line.PurchaseOrderLineId, line.InventoryItemId, inventoryItemById.GetValueOrDefault(line.InventoryItemId)?.Sku ?? "Unavailable", line.Sequence, line.AuthorizedQuantity, line.ShippedQuantity, line.UnitCost)).ToArray())).ToArray(),
                SupplierReturnShipments: supplierReturnShipments.Select(item => { var authorization = supplierReturnAuthorizationById[item.SupplierReturnAuthorizationId]; return new SupplierReturnShipmentSnapshot(item.Id, item.SupplierReturnAuthorizationId, authorization.ReturnNumber, item.SourceVendorBillId, item.SourceVendorBillId.HasValue ? billNumbersById.GetValueOrDefault(item.SourceVendorBillId.Value, "Unavailable") : string.Empty, authorization.VendorId, vendorNames.GetValueOrDefault(authorization.VendorId, "Unknown vendor"), item.ShipmentNumber, item.ShippedOn, item.Status, item.TotalAmount, item.CreatesVendorCredit, item.SourceAppliedAmount, item.AppliedAmount, item.RefundedAmount, item.TotalAmount - item.AppliedAmount - item.RefundedAmount, item.WarehouseId, item.BinId, InventoryLocationLabel(item.WarehouseId, item.BinId), item.JournalEntryId, item.ReversalJournalEntryId, item.ConcurrencyToken, supplierReturnShipmentLineLookup[item.Id].Select(line => new SupplierReturnShipmentLineSnapshot(line.Id, line.SupplierReturnAuthorizationLineId, line.InventoryReceiptLineId, line.PurchaseOrderLineId, line.InventoryItemId, inventoryItemById.GetValueOrDefault(line.InventoryItemId)?.Sku ?? "Unavailable", line.Sequence, line.Quantity, line.UnitCost, line.TotalAmount)).ToArray(), supplierReturnCreditApplicationLookup[item.Id].Select(application => new SupplierReturnCreditApplicationSnapshot(application.Id, application.VendorBillId, billNumbersById.GetValueOrDefault(application.VendorBillId, "Unavailable"), application.AppliedOn, application.Amount, application.Status, application.ConcurrencyToken)).ToArray(), supplierReturnCreditRefundLookup[item.Id].Select(refund => new SupplierReturnCreditRefundSnapshot(refund.Id, refund.BankAccountId, bankAccountNamesById.GetValueOrDefault(refund.BankAccountId, "Unavailable"), refund.Reference, refund.RefundDate, refund.Amount, refund.Status, refund.JournalEntryId, refund.ReversalJournalEntryId, refund.ConcurrencyToken)).ToArray()); }).ToArray()),
